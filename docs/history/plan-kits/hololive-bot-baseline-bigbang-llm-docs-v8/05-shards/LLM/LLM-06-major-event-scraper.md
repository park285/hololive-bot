...
```text
hololive/hololive-llm-sched/internal/service/majorevent/scraper/
```
...
- `run`
- `CheckLink`
- `withValidatedDialPolicy`
- `CheckEvents`
- `buildRetryRuns`
- `filterIncrementalEvents`
- `dedupeEvents`
...
scraper run, link check, retry build, incremental filter, dedupe를 별도 micro-shard로 처리합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-llm-sched/internal/service/majorevent/scraper/ \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-llm-sched/internal/service/majorevent/scraper
```
