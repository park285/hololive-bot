...
```text
hololive/hololive-llm-sched/internal/service/majorevent/scheduler/
```
...
send weekly/monthly 흐름을 prepare, render, deliver helper로 분리합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-llm-sched/internal/service/majorevent/scheduler/ \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-llm-sched/internal/service/majorevent/scheduler
```
