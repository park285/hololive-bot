# LLM-02 — llm scheduler bootstrap
...
component build 단계를 helper로 분리합니다. init order와 cleanup을 유지합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-llm-sched/internal/app/bootstrap_llm_scheduler.go \
  --output text \
  --sort-by score \
  --limit 50
```
