...
```text
hololive/hololive-llm-sched/internal/service/majorevent/summarizer/
```
...
schema/literal builder와 summarization behavior는 별도 micro-shard로 처리합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-llm-sched/internal/service/majorevent/summarizer/ \
  --output text \
  --sort-by score \
  --limit 50
```
