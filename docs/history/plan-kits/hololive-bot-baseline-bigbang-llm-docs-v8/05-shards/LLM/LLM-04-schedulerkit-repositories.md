# LLM-04 — schedulerkit and repositories
...
```text
hololive/hololive-llm-sched/internal/schedulerkit/runtime.go, hololive/hololive-llm-sched/internal/service/majorevent/repository_events.go
```
...
runtime run과 repository scan을 분리합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-llm-sched/internal/schedulerkit/runtime.go \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-llm-sched/internal/schedulerkit ./hololive/hololive-llm-sched/internal/service/majorevent/...
```
