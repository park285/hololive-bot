# AW-04 — alarm checker common helpers
...
```text
hololive/hololive-alarm-worker/internal/service/alarm/checker/common.go
```
...
- `tryLoadSubscriberRoomsByChannelBatched`
...
batching, fallback, schedule change detection을 분리합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-alarm-worker/internal/service/alarm/checker/common.go \
  --output text \
  --sort-by score \
  --limit 50
```
