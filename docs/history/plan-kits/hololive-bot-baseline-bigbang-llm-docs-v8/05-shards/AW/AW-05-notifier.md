...
send target resolution과 delivery call을 분리합니다. send order와 log key를 유지합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-alarm-worker/internal/service/alarm/checker/notifier.go \
  --output text \
  --sort-by score \
  --limit 50
```
