# AW-01 — alarm-worker app runtime
...
mode validation과 lifecycle를 분리합니다. scheduler start/shutdown order를 유지합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-alarm-worker/internal/app/ \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-alarm-worker/internal/app/...
```
