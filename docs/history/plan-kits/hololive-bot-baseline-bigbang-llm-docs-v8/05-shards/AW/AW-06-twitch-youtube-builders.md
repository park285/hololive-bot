# AW-06 — Twitch/YouTube notification builders
...
```text
hololive/hololive-alarm-worker/internal/service/alarm/checker/
```
...
notification builder를 input filtering, item build, append result로 분리합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-alarm-worker/internal/service/alarm/checker/ \
  --output text \
  --sort-by score \
  --limit 50
```
