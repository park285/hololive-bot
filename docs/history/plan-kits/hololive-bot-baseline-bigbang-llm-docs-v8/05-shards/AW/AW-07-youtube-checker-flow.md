# AW-07 — YouTube checker main flow
...
```text
hololive/hololive-alarm-worker/internal/service/alarm/checker/youtube_checker.go
```
...
YouTube Check는 단독 shard로 처리합니다. fetch/cache/notification path를 섞지 않습니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-alarm-worker/internal/service/alarm/checker/youtube_checker.go \
  --output text \
  --sort-by score \
  --limit 50
```
