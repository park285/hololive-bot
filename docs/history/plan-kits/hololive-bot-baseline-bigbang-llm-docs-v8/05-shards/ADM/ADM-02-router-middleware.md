# ADM-02 — admin-api router and middleware
...
```text
hololive/hololive-admin-api/internal/app/http/
```
...
route registration, domain handler validation, CORS guard를 각각 helper/table로 분리합니다. route path와 middleware order를 바꾸지 않습니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-admin-api/internal/app/http/ \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-admin-api/internal/app/http/...
```
