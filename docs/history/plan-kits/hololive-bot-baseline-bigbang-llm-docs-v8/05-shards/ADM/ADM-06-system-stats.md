# ADM-06 — admin-api system stats helper
...
```text
hololive/hololive-admin-api/internal/service/system/stats.go
```
...
command execution, output parsing, error handling을 helper로 분리합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-admin-api/internal/service/system/stats.go \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-admin-api/internal/service/system/...
```
