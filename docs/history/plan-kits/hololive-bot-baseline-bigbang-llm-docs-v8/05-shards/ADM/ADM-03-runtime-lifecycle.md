# ADM-03 — admin-api runtime lifecycle
...
```text
hololive/hololive-admin-api/internal/app/runtime/
```
...
lifecycle callbacks와 shutdown path를 named helper로 분리합니다. shutdown timeout과 server close order를 유지합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-admin-api/internal/app/runtime/ \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-admin-api/internal/app/runtime ./hololive/hololive-admin-api/internal/app/...
```
