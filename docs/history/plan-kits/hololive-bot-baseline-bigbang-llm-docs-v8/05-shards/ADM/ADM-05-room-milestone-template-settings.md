# ADM-05 — admin-api secondary handlers
...
```text
hololive/hololive-admin-api/internal/server/
```
...
각 파일별로 micro-shard를 나누십시오. streaming stats와 template preview는 같은 Worker에게 주지 않습니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-admin-api/internal/server/ \
  --output text \
  --sort-by score \
  --limit 50
```
