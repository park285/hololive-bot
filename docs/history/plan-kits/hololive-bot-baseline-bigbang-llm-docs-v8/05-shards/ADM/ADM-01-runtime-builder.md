# ADM-01 — admin-api runtime builder
...
```text
hololive/hololive-admin-api/internal/app/build_runtime.go
```
...
runtime construction 단계를 validate, infra clients, services, handlers, assembly helper로 분리합니다. cleanup-on-error order를 유지합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-admin-api/internal/app/build_runtime.go \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-admin-api/internal/app/...
```
