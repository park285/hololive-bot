# DSP-05 — dispatcher readiness/wakeup/render
...
```text
hololive/hololive-dispatcher-go/internal/app/runtime_http.go, hololive/hololive-dispatcher-go/internal/app/runtime_wakeup.go, hololive/hololive-dispatcher-go/internal/dispatch/render.go
```
...
readiness response와 wakeup wait behavior를 유지합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-dispatcher-go/internal/app/runtime_http.go \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-dispatcher-go/internal/app ./hololive/hololive-dispatcher-go/internal/dispatch
```
