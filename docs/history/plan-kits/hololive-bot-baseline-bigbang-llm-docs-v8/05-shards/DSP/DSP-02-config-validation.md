# DSP-02 — dispatcher config validation
...
validation을 server, iris, retry, mode, resource helper로 분리합니다. error prefix를 유지합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-dispatcher-go/internal/app/config.go \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-dispatcher-go/internal/app
```
