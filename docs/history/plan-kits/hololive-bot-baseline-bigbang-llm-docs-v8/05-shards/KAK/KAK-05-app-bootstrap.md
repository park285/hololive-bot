...
```text
hololive/hololive-kakao-bot-go/internal/app/bootstrap/
```
...
- `InitBotInfrastructure`
- `BuildBotDependencyModules`
...
infrastructure init과 dependency module build를 helper로 분리합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-kakao-bot-go/internal/app/bootstrap/ \
  --output text \
  --sort-by score \
  --limit 50
```
