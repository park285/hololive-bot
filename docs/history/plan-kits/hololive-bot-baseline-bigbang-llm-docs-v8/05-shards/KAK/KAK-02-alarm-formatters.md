...
```text
hololive/hololive-kakao-bot-go/internal/adapter/formatter_alarm.go
hololive/hololive-kakao-bot-go/internal/adapter/formatter_alarm_list.go
hololive/hololive-kakao-bot-go/internal/adapter/formatter_alarm_milestone.go
hololive/hololive-kakao-bot-go/internal/adapter/formatter_alarm_notification.go
```
...
header/body/item/footer rendering helper로 분리합니다. 출력 문자열을 유지합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-kakao-bot-go/internal/adapter/formatter_alarm \
  --output text \
  --sort-by score \
  --limit 50
```
