# KAK-01 — test_db_integration main
...
```text
hololive/hololive-kakao-bot-go/cmd/test_db_integration/main.go
```
...
main을 load config, connect, run checks, report helper로 분리합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-kakao-bot-go/cmd/test_db_integration/main.go \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-kakao-bot-go/cmd/test_db_integration
```
