# KAK-07 — bot core lifecycle/transport
...
```text
hololive/hololive-kakao-bot-go/internal/bot/
```
...
파일별로 micro-shard를 나누십시오. async command와 handoff wait는 같은 Worker에게 주지 않습니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-kakao-bot-go/internal/bot/ \
  --output text \
  --sort-by score \
  --limit 50
```
