...
```text
hololive/hololive-kakao-bot-go/internal/app/http, hololive/hololive-kakao-bot-go/internal/app/runtime
```
...
router and runtime lifecycle를 분리합니다. HTTP/3 behavior와 shutdown order를 유지합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-kakao-bot-go/internal/app/http \
  --output text \
  --sort-by score \
  --limit 50
```
