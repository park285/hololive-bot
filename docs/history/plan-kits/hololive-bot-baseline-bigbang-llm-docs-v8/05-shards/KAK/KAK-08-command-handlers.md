...
```text
hololive/hololive-kakao-bot-go/internal/command/
```
...
- `handleAdd`
- `parseAlarmTypes`
- `buildChzzkLiveStreams`
- `resolveMember`
- `buildGroupEntries`
- `normalizeMemberGroup`
- `extractUnitValues`
...
alarm/live/member group handlers를 별도 micro-shard로 처리합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-kakao-bot-go/internal/command/ \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-kakao-bot-go/internal/command
```
