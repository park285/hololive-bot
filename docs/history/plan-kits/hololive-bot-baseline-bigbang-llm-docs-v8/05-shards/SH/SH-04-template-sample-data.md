...
```text
hololive/hololive-shared/pkg/domain/template_sample_data_core.go
hololive/hololive-shared/pkg/domain/template_sample_data_major_event.go
hololive/hololive-shared/pkg/domain/template_sample_data_member_news.go
```
...
map literal을 category helper로 분리합니다. returned shape를 유지합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-shared/pkg/domain/template_sample_data_ \
  --output text \
  --sort-by score \
  --limit 50
```
