...
- `ProvideMemberCache`
- `BuildInfraModule`
- `hasExplicitAndImplicitRegistrations`
...
provider module and renew loop를 helper로 분리합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-shared/pkg/providers/ \
  --output text \
  --sort-by score \
  --limit 50
```
