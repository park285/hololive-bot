...
- `clearSubscriberCacheNamespace`
- `WarmSubscriberCacheFromAlarms`
- `writeWarmSetMap`
- `writeWarmHash`
...
cache clear/write order, empty sentinel, partial failure handling을 유지합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-shared/pkg/service/alarm/cache_warm.go \
  --output text \
  --sort-by score \
  --limit 50
```
