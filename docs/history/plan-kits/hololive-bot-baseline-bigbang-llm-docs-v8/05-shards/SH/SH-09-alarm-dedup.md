...
```text
hololive/hololive-shared/pkg/service/alarm/dedup/
```
...
- `tryClaim`
- `cleanupExpired`
- `TryClaimNotificationScheduleChange`
- `detectLegacyUpcomingEventScheduleChange`
- `MarkAsNotified`
- `IsAlreadyNotifiedForSchedule`
...
claim, cleanup, schedule-change detection, mark/is-notified order를 유지합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-shared/pkg/service/alarm/dedup/ \
  --output text \
  --sort-by score \
  --limit 50
```
