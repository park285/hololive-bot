# SH-03 — shared domain scanner/stats/youtube content
...
- `Scan`
- `ValidateLegacyRoute`
- `NormalizeStatsPeriodToken`
- `resolveNamedStatsPeriod`
- `ToTemplateKey`
...
scanner and normalization branch를 helper로 분리합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-shared/pkg/domain/ \
  --output text \
  --sort-by score \
  --limit 50
```
