...
```text
hololive/hololive-shared/pkg/config/
```
...
- `Validate`
- `validateScraperPublishedAtResolverConfig`
- `validatePostgresSSLMode`
- `validateServerTransports`
...
config build and validation helper로 분리합니다. env/default behavior를 유지합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-shared/pkg/config/ \
  --output text \
  --sort-by score \
  --limit 50
```
