# SH-15 — chzzk/holodex/member/twitch clients
...
```text
hololive/hololive-shared/pkg/service/chzzk, hololive/hololive-shared/pkg/service/holodex, hololive/hololive-shared/pkg/service/member, hololive/hololive-shared/pkg/service/twitch
```
...
API pagination/status check/scraper/repository helper로 분리합니다. 최신 inventory에서 package별 auto-shard로 쪼갭니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-shared/pkg/service/chzzk \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-shared/pkg/service/chzzk ./hololive/hololive-shared/pkg/service/holodex ./hololive/hololive-shared/pkg/service/member ./hololive/hololive-shared/pkg/service/twitch
```
