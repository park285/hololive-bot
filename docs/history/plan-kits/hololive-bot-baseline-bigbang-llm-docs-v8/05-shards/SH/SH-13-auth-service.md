...
```text
hololive/hololive-shared/pkg/service/auth/
```
...
token/session invalidation, password verification, error mapping을 유지합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-shared/pkg/service/auth/ \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-shared/pkg/service/auth
```
