# SH-01 — shared internal dbx and retry
...
```text
hololive/hololive-shared/internal/
```
...
- `Connect`
- `parseQueryExecMode`
- `normalizeQueryExecMode`
- `ShouldFallbackToLocalhost`
- `OpenWithRetry`
- `WithRetry`
...
DB connection/retry fallback helper를 분리합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-shared/internal/ \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-shared/internal/...
```
