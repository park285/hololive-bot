# Live-status 스크래퍼 폴백 페이싱 플랜 코드 리뷰 및 v3 개선안 (historical)

> Historical 2026-06-23 snapshot. Current collector fleet identity is `youtube-collector`. `youtube-producer` / `hololive-youtube-producer` in this document is a retired name.
