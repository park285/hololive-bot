# YouTube Producer Active-Active 다중 워커 최적화 코드레벨 결정서 (historical)
...
Scheduler
  ...
  ├─ GlobalBudgetLimiter: source별 전역 admission/in-flight budget 제어
...
Valkey Lua script로 in-flight counter와 optional window counter를 원자 처리합니다.
...
Modify:
  ...
  hololive/hololive-shared/pkg/service/youtube/poller/internal/scheduler_worker.go
  hololive/hololive-shared/pkg/service/youtube/poller/internal/scheduler_reschedule.go
  hololive/hololive-shared/pkg/service/youtube/poller/internal/metrics.go
  hololive/hololive-shared/pkg/service/youtube/poller/internal/scheduler_test.go
...
- active-active off일 때 기존 single-owner 동작 유지
- active-active on일 때 `JobRunGuard` claim/cooldown invariant 유지
- `BudgetLimiter=nil` 또는 `YOUTUBE_PRODUCER_GLOBAL_BUDGET_ENABLED=false`이면 기존 scheduler 동작 유지
- budget denied 시 poller 미실행, claim release, retryAfter reschedule
- budget backend error 시 fail-closed readiness와 claim release
- lease renew loop가 budget reserve/local wait 중에도 동작
- 기존 scraper/Holodex request limiter 유지
- 모든 registration에 `BudgetProfile` 존재
- Phase 1 검증 명령 통과
...
```text
hololive/hololive-shared/pkg/service/youtube/poller/internal/batch_poller.go
hololive/hololive-shared/pkg/service/youtube/poller/internal/scheduler_batch.go
hololive/hololive-shared/pkg/service/youtube/poller/internal/scheduler_dispatch.go
hololive/hololive-shared/pkg/service/youtube/poller/internal/scheduler_worker.go
hololive/hololive-shared/pkg/service/youtube/poller/internal/pollers/live_poller.go
...
- [ ] `BatchPoller` optional interface와 `BatchPollResult` helper를 추가한다.
- [ ] `LivePoller.PollBatch`가 입력 channel마다 result entry를 반환하도록 구현한다.
- [ ] `LivePoller.PollBatch`에서 stream grouping, baseline, `markEndedSessions`, partial failure 계약을 지킨다.
- [ ] scheduler는 due live jobs를 worker-side micro-batch로 모으되, acquired claim만 batch input으로 넘긴다.
- [ ] acquired claim마다 renew loop를 즉시 시작하고, channel별 finish에서 `MarkCompleted`/`Release`를 분리한다.
- [ ] Holodex budget은 batch 단위로 reserve하고, fallback scraper budget은 fallback path에서 별도 reserve한다.
- [ ] `YOUTUBE_PRODUCER_LIVE_BATCH_ENABLED=false`이면 기존 channel 단위 `Poll`을 사용한다.
- [ ] batch size/duration/channel result metric을 추가한다.
...
- channel별 baseline과 `markEndedSessions` invariant 유지
- renew lost channel은 해당 channel 실패로 처리
...
- [ ] scraper error classification을 source cooldown writer와 연결한다.
- [ ] 429/403/parser drift/transport 별 cooldown TTL 정책을 구현한다.
- [ ] source cooldown admission denied는 readiness failure가 아니라 scheduler reschedule로 처리한다.
- [ ] backfill source budget을 primary와 분리하되 source global hard cap은 공유한다.
- [ ] backfill이 primary budget을 침범하지 않도록 validator를 추가한다.
- [ ] `YOUTUBE_PRODUCER_SOURCE_COOLDOWN_ENABLED=false`이면 cooldown gate를 비활성화한다.
- [ ] source cooldown metric과 readiness payload를 추가한다.
...
```text
1. 기존 scraper/Holodex request limiter를 제거해야 할 필요가 생김
2. runtime token bucket을 켜야만 요구사항을 만족할 수 있음
3. readiness HTTP status contract를 바꿔야 함
4. SCRAPER_SCHEDULER_WORKER_COUNT 외 새 worker env alias가 필요함
5. LivePoller batch가 channel별 markEndedSessions invariant를 보존할 수 없음
6. Valkey Lua reservation cleanup에서 inflight 보정이 idempotent하지 않음
```
...
func (s *Scheduler) executeJob(ctx context.Context, job *Job, workerID int) {
    ...
    if err != nil {
        ...
        s.markBudgetBackendUnavailable(err)
...
성공 channel만 markBaselineComplete
markEndedSessions는 channel별 stream set만 사용
renew lost channel은 해당 channel 실패 처리
```
