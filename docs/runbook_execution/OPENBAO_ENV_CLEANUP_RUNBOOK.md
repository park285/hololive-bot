...
for f in "${profile_files[@]}"; do
  [ -f "$f" ] || continue
  awk '
    /(^|[[:space:]])(export[[:space:]]+)?(DB_PASSWORD|CACHE_PASSWORD|IRIS_BOT_TOKEN|IRIS_WEBHOOK_TOKEN|ADMIN_PASS_BCRYPT|SESSION_SECRET|ALARM_DISPATCH_|POSTGRES_|CACHE_|YOUTUBE_PRODUCER_RUNTIME_ALLOWED|NOTIFICATION_EGRESS_ROLE|NOTIFICATION_SCHEDULER_ROLE)=/ {
      print FILENAME ":" FNR
    }
  ' "$f"
done
...
unset YOUTUBE_PRODUCER_RUNTIME_ALLOWED
