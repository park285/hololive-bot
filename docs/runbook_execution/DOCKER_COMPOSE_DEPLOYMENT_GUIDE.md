...
- 기존 state/control: `holo-postgres` (`<tailnet-central>:5433`), `valkey-cache` (`<tailnet-central>:6379`), CLIProxy (`http://<tailnet-central>:8787/v1`), `hololive-bot`, admin 계열 서비스
...
curl -fsS http://<tailnet-central>:30001/health
```

- `HOLOLIVE_BOT_PORT_BIND_IP`를 설정하지 않으면 `hololive-bot`은 기존 운영 Tailscale IP인 `<tailnet-central>:30001`에도 publish 됩니다.
