...
split-host 구성에서는 producer AP runtime container만 원격 호스트에서 실행하고, shared state/control은 기존 `kapu` (`100.100.1.3`)에 둡니다. 토폴로지는 3-way active-active입니다.

- Osaka runtime: `youtube-producer-a` (`kapu-iris-osaka-1`, `100.100.1.7`, overlay `docker-compose.osaka.yml`, port `30005`)
- Seoul runtime: `youtube-producer-b` (`100.100.1.5`, overlay `docker-compose.seoul.yml`, port `30015`)
...
- 원격 AP env file: OpenBao Agent가 렌더링한 `/run/hololive-bot/env` (`COMPOSE_ENV_FILE=/run/hololive-bot/env`)
...
$SSH_OSAKA 'cd ~/hololive-bot && sudo -n env COMPOSE_ENV_FILE=/run/hololive-bot/env COMPOSE_PROFILES=oracle ./scripts/deploy/compose.sh -f docker-compose.prod.yml -f docker-compose.osaka.yml up -d --no-deps --remove-orphans youtube-producer-a'

SSH_SEOUL='ssh -i /home/kapu/gemini/hololive-bot/KR.key -o IdentitiesOnly=yes -o HostKeyAlias=100.100.1.5 ubuntu@100.100.1.5'
$SSH_SEOUL 'cd ~/hololive-bot && sudo -n env COMPOSE_ENV_FILE=/run/hololive-bot/env COMPOSE_PROFILES=oracle ./scripts/deploy/compose.sh -f docker-compose.prod.yml -f docker-compose.seoul.yml up -d --no-deps --remove-orphans youtube-producer-b'
```
...
원격 AP 호스트에는 source tree를 상시 보관하지 않습니다. 운영 디렉터리에는 `scripts/deploy/ap-rsync-files.txt` 매니페스트가 동기화한 compose/build 파일과 `runtime-config/`, `data/`, `logs/`만 두고 secret env는 `/run/hololive-bot/env`에서만 읽습니다. image는 해당 호스트가 동기화된 build context에서 직접 build합니다.
...
- `hololive-alarm-worker` → alarm scheduler / checker / queue publish and proactive egress
...
./scripts/deploy/compose.sh -f docker-compose.prod.yml logs -f hololive-admin-api
...
./scripts/logs/logs.sh tail llm --since 30m
./scripts/logs/logs.sh backfill llm --since 24h
./scripts/logs/logs.sh stream start
./scripts/logs/logs.sh prune
```

- compose 런타임에서는 `LOG_DIR=/app/logs`로 설정해 host `./logs/bot.log`, `./logs/admin-api.log`, `./logs/alarm-worker.log`, `./logs/llm-scheduler.log`, `./logs/youtube-producer.log`에 파일 미러링합니다.
