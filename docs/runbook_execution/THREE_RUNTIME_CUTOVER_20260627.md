...
| PR | main 커밋 | 내용 |
|---|---|---|
| #146 | `e45a469a` | three-runtime 통합 후 빌드/마이그레이션 경로 정리: Dockerfile dead-copy 제거, migration mount 경로 정정, `check-removed-runtime-build-paths.sh` guard 추가(삭제된 옛 모듈 경로 참조 회귀 차단), ci-boundary gate 보강. docker build 3종·compose render·migration mount·ci-boundary 447 PASS로 검증 |
...
- **guard test 제외 축소**: `check-removed-runtime-build-paths.sh`(#146) 등 guard의 test 제외 범위를 최소화 — 후속.
