...
- 마이그레이션 우선순위: **alarm_state DB adapter → JobRunGuard Valkey adapter → ingestion runtime singleton lease 재평가**. outbox reuse cache 는 token-as-timestamp + SQL `FOR UPDATE` 기반 get-or-compute 패턴이라 현재 `ReuseCache.Claim` (mutation API) 와 incompatible. 별도 `ResolveClaim` (get-or-compute) helper 신설 후 재진입한다 (자세한 사유는 `docs/agent-workflows/plans/...` 또는 commit 1dd1b1c0 의 후속 검토 참고).
