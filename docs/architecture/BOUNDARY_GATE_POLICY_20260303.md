...
1. `scripts/architecture/ci-boundary-gate.sh`
   ...
   - Go import graph 산출
2. `scripts/architecture/ci-boundary-gate.sh`
...
4. `scripts/architecture/ci-boundary-gate.sh`
...
1. 표준 진입점 실행 및 성공
   - `./scripts/architecture/ci-boundary-gate.sh`
2. 필수 게이트 통과 확인
   - `M0`: `ci-boundary-gate.sh` 내부 M0 단계
   - `M1`: `ci-boundary-gate.sh` 내부 M1 단계
   - `M4`: `check-go-module-loc.sh`
   - `M6`: `ci-boundary-gate.sh` 내부 M6 단계
3. 증적 보관
