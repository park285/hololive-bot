...
| `alarm.state.read` | `alarm-worker` (data owner) | `youtube-producer` | PostgreSQL table read | `alarms` read-only (`GetAllChannelIDs`, `LoadAll`) | `hololive/hololive-shared/pkg/service/alarmread` | Go interface, unversioned | producer poll-target/baseline tests; `check-repository-ownership.sh` | `contracts/alarm.md` |
