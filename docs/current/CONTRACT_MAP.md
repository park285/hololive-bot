...
| `alarm.state.read` | `alarm-worker` (data owner) | `hololive-api` | PostgreSQL table read | `alarms` read-only (`GetAllChannelIDs`, `LoadAll`) | `hololive/hololive-shared/internal/service/alarmread` (injected through `pkg/providers.ProvideAlarmReader`) | Go interface, unversioned | `check-repository-ownership.sh` | `contracts/alarm.md` |
