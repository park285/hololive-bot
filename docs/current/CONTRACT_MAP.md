...
현재 내부 HTTP JSON, Valkey queue, Valkey Pub/Sub, Iris external boundary와 public redirect 계약을 한눈에 추적합니다. RPC/gRPC 전환은 이 문서 범위가 아닙니다.
...
| `alarm.state.read` | `alarm-worker` (data owner) | `hololive-api` | PostgreSQL table read | `alarms` read-only (`GetAllChannelIDs`, `LoadAll`) | `hololive/hololive-shared/pkg/service/alarmread` | Go interface, unversioned | `check-repository-ownership.sh` | `contracts/alarm.md` |
...
| `shortlink.youtube` | `hololive-api` | browsers, KakaoTalk scraper, `alarm-worker` renderer | External HTTPS redirect | `GET`, `HEAD` `/l/:videoID` | `hololive/hololive-shared/pkg/contracts/shortlink` | route constants, unversioned | route, origin, scraper rejection, render tests | `contracts/shortlink.md` |
| `settings.update` | `hololive-api` current publisher through admin-plane settings update paths | `hololive-api`, `alarm-worker`, `youtube-collector`, ingestion runtimes where subscriber configured | Valkey Pub/Sub | `config:update` | `hololive/hololive-shared/pkg/contracts/settings` | `ConfigUpdateVersionV1 = 1`; message has no `version` field | settings/configsub tests | `contracts/settings.md` |
