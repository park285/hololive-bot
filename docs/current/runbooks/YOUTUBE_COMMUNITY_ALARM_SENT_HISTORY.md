...
지정한 닫힌 observation window에서 실제 발송 완료 상태로 확정된 유튜브 커뮤니티 알람 게시물 목록을 검증 원본으로 수집할 때 사용하는 운영 절차입니다.

## Scope

- 대상은 `COMMUNITY_POST` observation baseline에 포함된 게시물만 포함합니다.
...
- `post_id`: observation baseline에서 고정된 canonical community post identifier입니다.
