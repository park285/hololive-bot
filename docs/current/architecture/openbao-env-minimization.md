...
| Repo / 위치 | 역할 | 호스트 |
|---|---|---|
| `hololive-bot/` | compose 계약, deploy 스크립트, repo security contract test | central(`<tailnet-central>`) + Osaka AP + Seoul AP |
| `chat-bot-go-kakao/` | compose 계약, config 로더, redeploy 스크립트 | central(`<tailnet-central>`) |
...
- AP 호스트별 PKI `ip_sans` 적합성(현행 hcl은 `ip_sans=<tailnet-central>` 고정 — AP 호스트 렌더 시 올바른 SAN인지).
...
- AP unit은 `User=openbao-agent-hololive-bot-ap`, `Group=opc`로 실행한다. AP producer image가 `1000:1000` non-root로 실행되므로 cert/key는 `openbao-agent-hololive-bot-ap:opc 0640`으로 렌더되어 container-readable하고, env 파일은 `openbao-agent-hololive-bot-ap:opc 0600`으로 유지된다.
...
- 인증서 name/IP set은 `holo-postgres`, `host.docker.internal`, `localhost`, `<tailnet-central>`, `127.0.0.1`이다. PostgreSQL server key는 client CA bundle 디렉토리와 분리된 `postgres-tls/`에 둔다.
