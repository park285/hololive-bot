...
기준은 `main`의 `d5f54aab6ff88d3dc30122187c3e9296bbffd630`이다. 이 문서는 PR #505의 런타임 변경과 남은 검증을 설명한다. 이전 27파일 제안 패키지를 그대로 게시한 것이 아니다. 코드를 재검토해 17개 코드·테스트 파일과 본 문서로 범위를 한정했다.
...
| dbx 배치 | 8 | 크기·순서, fallback, 실행·Close 오류, panic 회수, 취소, nil 결과 |
| 실제 저장 호출부 | 6 | session/head 순서·인자, 두 SQL의 RowsAffected, DB 롤백, 묶음 경계 오류 |

작성 환경에서 직접 실행한 것은 동일한 coverage helper의 독립 테스트 4개와 race 검사이다(Go 1.23.2/amd64). 15개 Go 파일의 parser 검사와 gofmt 검사도 실행했다. parser 검사는 패키지 타입 검사의 대체물이 아니다.

전체 Go 1.27 패키지 검증은 변경하지 않은 저장소의 PR CI가 실행한다. 최종 commit에 해당하는 CI 상태와 세부 로그를 PR에서 확인해야 한다. 첫 commit의 성공을 후속 commit의 성공으로 간주하지 않는다. 로컬 full pre-push, NilAway, 운영 부하·ARM64 벤치마크는 별도 검증 사항이다.
