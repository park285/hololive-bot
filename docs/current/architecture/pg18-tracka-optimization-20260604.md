...
`docker-compose.prod.yml`의 holo-postgres `command:` 블록으로 명시화, 컨테이너 recreate로 적용:

- `shared_preload_libraries=pg_stat_statements`, `pg_stat_statements.track=all`, `pg_stat_statements.max=5000`, `compute_query_id=on`, `track_io_timing=on`
- `track_wal_io_timing`은 PG18에서 제거된 GUC라 사용 금지 — WAL I/O는 `pg_stat_io(object='wal')`로 관측 (검증: writes 누적 확인됨)
...
예산 근거: pool max = 4서비스×기본 25 + alarm-worker 8 = 이론상 108 > `max_connections=100`(잠재 리스크, 아래 후속), 실측 동시 커넥션 33.
...
| outbox/delivery claim CTE | Seq Scan — 테이블 극소(10~61행)라 planner가 의도적으로 seq 선택, partial index는 규모 증가 시 발동 | 조건부 |
...
1. pool max 이론상 합(108) > `max_connections=100` — 포화 근접 시 `POSTGRES_POOL_MAX_CONNS` 서비스별 명시 하향 또는 max_connections 상향 검토 (실측 33이라 현재 비위험).
2. AIO 파라미터(`io_workers`/`effective_io_concurrency`)는 disk read가 실제 발생할 때 `pg_stat_io` 기반 재조정.

## 롤백

`docker-compose.prod.yml`의 holo-postgres `command:` 블록 git revert + `compose-redeploy-service.sh holo-postgres` recreate. 데이터 볼륨(`holo-pg-data`) 무관. extension은 잔존해도 무해(`shared_preload_libraries` 제거 시 자동 비활성).
