...
| outbox claim CTE | (재캡처, literal params) `idx_yno_status_created` Bitmap Index Scan, `Index Searches: 1`, 0.141ms — GENERIC_PLAN의 seq는 generic-plan 비관치였음 | 효과 있음 |
...
1. pool max 이론상 합(108) > `max_connections=100`, 그리고 worst-case `work_mem` 과약정(108conn × 4MB × 노드 배수 vs 512MB cap) — 포화 근접 시 `POSTGRES_POOL_MAX_CONNS` 서비스별 명시 하향 또는 cap/max_connections 재검토 (실측 33conn·RSS 21%라 현재 비위험).
