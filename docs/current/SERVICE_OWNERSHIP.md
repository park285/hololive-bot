...
| `youtube-producer` | YouTube producer AP runtime: primary/backfill polling, outbox production, 3-way active-active poll coordination (Osaka a + Seoul b + main-host c), readiness, and Holodex photo sync (AP-A/AP-C, singleton lease) | YouTube poller/outbox production, active-active coordination/readiness, photo sync runtime | PostgreSQL, Valkey | bot command routing, proactive notification egress, alarm dispatch queue consumption | `services/youtube-producer.md` |
