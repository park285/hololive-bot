...
- Alarm-worker runtime은 기본값 `BOT_MARKDOWN_REPLIES=false`에서 오픈채팅도 기존 `kakaoformat.Render` 변환 렌더러를 사용합니다. 명시적으로 `true`를 설정한 경우에만 확인된 오픈채팅에서 네이티브 Markdown message lane을 사용합니다.
