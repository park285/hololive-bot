...
| `youtube-collector` | `hololive-youtube-collector` | `youtube-collector` | `youtube-collector` | 30045 | `https://127.0.0.1:30045/health` | `services/youtube-collector.md` | `runbooks/youtube-collector.md` |
...
| `admin-dashboard` | Dashboard (Go backend + embedded frontend) | Not part of the 4 app runtime set |
...
- Keep service docs and runbook links valid for all 4 runtime rows.
...
- Architecture: Go single-language runtime (4 app runtimes: hololive-api + alarm-worker + youtube-producer + youtube-collector). `hololive-api` hosts the bot/admin/llm planes in one process on ports 30001/30003/30006.
- Central host default `docker compose up -d` starts `hololive-api` + `hololive-alarm-worker` + `youtube-collector`. `youtube-producer` is AP-owned and gated behind `COMPOSE_PROFILES=oracle` (central `c`) or the per-host AP overlays (`docker-compose.{osaka,osaka2,seoul,main-ap}.yml`); it is intentionally absent from a profile-less central `up`. `youtube-collector` is a required central Community singleton; AP overlays pin it to profile `central-only` so it does not start on a/b/d.
