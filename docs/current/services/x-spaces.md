...
Node 모의 응답 테스트는 비용 경로·redirect·응답 크기·호스트·상태·인증 오류와, 고정된 요청 ID 라이브러리가 실제로 요청하는 앱 셸 주소의 허용 여부를 검사한다. 실패 보고 테스트는 예외 원문·임의 오류 이름이 결과 문서에 남지 않는지 검사하고, Go 테스트는 허용 목록 밖 진단의 거부와 종료 상태 보존을 검사한다. Go DB 테스트는 후보 실패 보존, 세대 경쟁, 암호문 변조, 재시작·제목 변경 후 중복 제거를 검사한다. 실계정 성공과 지정 방 발송 확인은 별도 실제 증거로 기록한다.
