# SH-12 — alarm targets/tier/client/repository
...
- `doJSON`
- `scanAlarms`
- `ResolveChannelSubscribersByType`
- `loadChannelSubscriberAlarms`
- `extractSubscriberIDsByType`
- `ComputeNextCheckAt`
- `UpdateChannelState`
- `ValidateChannelContentAlarmTargetDefinitions`
...
target resolve, tier policy, repository scan, client HTTP helper를 분리합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-shared/pkg/service/alarm/ \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-shared/pkg/service/alarm/...
```
