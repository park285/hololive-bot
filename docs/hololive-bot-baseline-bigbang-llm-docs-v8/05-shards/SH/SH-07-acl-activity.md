# SH-07 — ACL and activity services
...
```text
hololive/hololive-shared/pkg/service/acl, hololive/hololive-shared/pkg/service/activity
```
...
ACL storage/cache sync and activity log query를 분리합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-shared/pkg/service/acl \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-shared/pkg/service/acl ./hololive/hololive-shared/pkg/service/activity
```
