...
```text
hololive/hololive-shared/pkg/service/alarm/queue/
```
...
queue drain, delayed retry, schedule retry, requeue, metrics, publish batch를 helper로 분리합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-shared/pkg/service/alarm/queue/ \
  --output text \
  --sort-by score \
  --limit 50
```
