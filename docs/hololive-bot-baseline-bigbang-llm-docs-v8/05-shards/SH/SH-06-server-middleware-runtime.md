# SH-06 — server middleware/runtime/helpers
...
```text
hololive/hololive-shared/pkg/server/
```
...
- `Summary`
- `translateWindowsVersion`
- `ToLogFields`
- `NewIPAllowList`
- `AdminIPAllowMiddleware`
- `LoggerMiddleware`
- `shouldSkipPath`
- `NewTriggerRuntimeRouter`
- `NewRuntimeRouter`
- `AsMap`
- `GetChannel`
- `GetActiveMemberIndex`
- `GetChannelStats`
- `triggerChannelStatsRefreshAsync`
...
middleware behavior, route registration, stream handler response를 유지합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-shared/pkg/server/ \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-shared/pkg/server/...
```
