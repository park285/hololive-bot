...
- `insertEvents`
- `insertDeliveries`
- `validateEventPayloadRoomAgnostic`
- `terminal`
- `terminalUpdates`
...
consumer, insert events, insert deliveries, terminal updates를 각각 micro-shard로 처리합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-shared/pkg/service/alarm/dispatchoutbox/ \
  --output text \
  --sort-by score \
  --limit 50
```
