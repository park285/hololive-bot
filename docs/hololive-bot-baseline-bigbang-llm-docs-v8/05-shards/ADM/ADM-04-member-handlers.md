# ADM-04 — admin-api member handlers
...
```text
hololive/hololive-admin-api/internal/server/api_member.go
```
...
decode, validation, service call, response write를 helper로 분리합니다. status code와 JSON field를 유지합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-admin-api/internal/server/api_member.go \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-admin-api/internal/server
```
