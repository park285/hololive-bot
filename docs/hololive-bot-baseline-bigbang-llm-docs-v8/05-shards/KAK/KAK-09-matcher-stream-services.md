# KAK-09 — matcher and stream services
...
```text
hololive/hololive-kakao-bot-go/internal/service/
```
...
- `tryExactValkeyMatch`
- `tryPartialAliasMatch`
- `hydrateChannel`
- `buildSnapshot`
- `exactNameMembers`
- `trySnapshotPartialAliasMatch`
- `mergeLiveStreams`
- `mergeUpcomingStreams`
- `mergeStelliveUpcomingStreams`
- `GetChannelSchedule`
- `buildChzzkScheduleStreams`
- `compareScheduleStreams`
- `GenerateRoomDigest`
- `FindByChannelAndScheduledMinute`
...
matcher와 stream service는 package별로 micro-shard를 나눕니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-kakao-bot-go/internal/service/ \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-kakao-bot-go/internal/service/matcher ./hololive/hololive-kakao-bot-go/internal/service/streamfeed ./hololive/hololive-kakao-bot-go/internal/service/streamschedule
```
