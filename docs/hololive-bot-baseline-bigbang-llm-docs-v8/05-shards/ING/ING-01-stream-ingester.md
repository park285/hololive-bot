...
- `inventory에서 발견된 stream-ingester over-budget 함수`
...
runtime/ops 함수는 R5처럼 작게, dataset render 함수는 section helper로 분리합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-stream-ingester/ \
  --output text \
  --sort-by score \
  --limit 50
```
