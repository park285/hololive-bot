# DSP-08 — dispatcher retry backoff
...
attempt normalization, exponential cap, jitter calculation을 helper로 분리합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-dispatcher-go/internal/dispatch/dispatcher.go \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-dispatcher-go/internal/dispatch
```
