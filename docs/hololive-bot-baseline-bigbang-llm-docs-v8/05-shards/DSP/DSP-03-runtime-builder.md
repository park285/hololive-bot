# DSP-03 — dispatcher runtime builder
...
client/resource set, consumer, iris, dispatcher, managed cleanup으로 분리합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-dispatcher-go/internal/app/runtime.go \
  --output text \
  --sort-by score \
  --limit 50
```
