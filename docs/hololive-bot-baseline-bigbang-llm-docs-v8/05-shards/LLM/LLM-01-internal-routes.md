# LLM-01 — internal route registration
...
```text
hololive/hololive-llm-sched/internal/app/api_internal_majorevent.go
hololive/hololive-llm-sched/internal/app/api_internal_membernews.go
```
...
- `registerMajorEventInternalRoutes`
- `registerMemberNewsInternalRoutes`
...
route registration helper를 분리합니다. path와 method를 유지합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-llm-sched/internal/app/api_internal_ \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-llm-sched/internal/app/...
```
