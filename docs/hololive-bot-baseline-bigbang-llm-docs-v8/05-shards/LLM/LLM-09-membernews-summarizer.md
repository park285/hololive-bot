...
```text
hololive/hololive-llm-sched/internal/service/membernews/summarizer/
```
...
- `Summarize`
- `validateAndBuildDigestFromResponse`
- `categoryLabel`
- `normalizeCategory`
...
schema builder와 response validation은 별도 micro-shard로 처리합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-llm-sched/internal/service/membernews/summarizer/ \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-llm-sched/internal/service/membernews/summarizer
```
