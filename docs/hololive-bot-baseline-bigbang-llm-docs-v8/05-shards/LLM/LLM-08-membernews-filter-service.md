# LLM-08 — membernews filter/service/scheduler
...
```text
hololive/hololive-llm-sched/internal/service/membernews/
```
...
filter, repository warmup, digest dispatch, source validator는 각각 별도 micro-shard로 처리합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-llm-sched/internal/service/membernews/ \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-llm-sched/internal/service/membernews/...
```
