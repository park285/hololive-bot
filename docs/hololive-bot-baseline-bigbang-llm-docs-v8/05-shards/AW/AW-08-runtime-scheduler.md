...
```text
hololive/hololive-alarm-worker/internal/service/alarm/scheduler/runtime_scheduler.go
```
...
tick interval, error-triggered recovery, context cancellation을 유지합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-alarm-worker/internal/service/alarm/scheduler/runtime_scheduler.go \
  --output text \
  --sort-by score \
  --limit 50
```
