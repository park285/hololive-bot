# AW-09 — alarm cache recovery scheduler
...
```text
hololive/hololive-alarm-worker/internal/service/alarm/scheduler/runtime_scheduler_cache_recovery.go
```
...
- `runAlarmCacheRecoveryLoop`
- `recoverAlarmCacheIfRegistryEmpty`
- `syncPlatformMappingsIfMissing`
...
cache recovery loop, registry empty check, platform sync를 helper로 분리합니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-alarm-worker/internal/service/alarm/scheduler/runtime_scheduler_cache_recovery.go \
  --output text \
  --sort-by score \
  --limit 50
```
...
```bash
go test ./hololive/hololive-alarm-worker/internal/service/alarm/scheduler
```
