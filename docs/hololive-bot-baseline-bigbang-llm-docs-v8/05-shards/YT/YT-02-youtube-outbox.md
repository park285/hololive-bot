...
claim, delivery, timeline, dispatcher send semantics를 유지합니다. auto-shard 우선입니다.
...
python3 scripts/architecture/check-function-budget.py \
  --root . \
  --report-over-budget \
  --include-prefix hololive/hololive-shared/pkg/service/youtube/outbox/ \
  --output text \
  --sort-by score \
  --limit 50
```
