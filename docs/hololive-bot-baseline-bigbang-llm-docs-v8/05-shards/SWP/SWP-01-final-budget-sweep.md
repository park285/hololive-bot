# SWP-01 — final over-budget sweep
...
남은 over-budget 함수 top 1~5를 exact micro-shard로 반복 처리합니다.
...
```bash
python3 scripts/architecture/check-function-budget.py --root . --report-over-budget --output text --sort-by score --limit 200
```
