...
`alarm.http` is in a staged provider migration. `alarm-worker` now registers the target `/internal/alarm/*` provider route set through the shared alarm route registrar, while `admin-api` remains as a compatibility provider until all callers are cut over.

## Current State

- `alarm-worker` registers `hololive-shared/pkg/service/alarm.NewInternalRouteRegistrar` when `AlarmCRUD` is configured.
- `admin-api` uses the same shared route registrar for compatibility registration during the migration window.
...
- Staged duplication is allowed only while the full route set exists on both sides.
...
- Runtime packages should use the shared alarm route registrar instead of duplicating Gin route wiring locally.
