...
if [[ $# -ne 2 || ! "$1" =~ ^sha256:[0-9a-f]{64}$ || ! "$2" =~ ^sha256:[0-9a-f]{64}$ ]]; then
  echo 'usage: test-admin-bigbang-js-budget.sh <native-baseline-image> <native-candidate-image>' >&2
  exit 2
fi
ADMIN_LAB_ID="admin-lab-js-budget-$$-$(date +%s)"
cleanup() {
  local id
...
systemd-run --user --quiet --wait --pipe --collect --unit="${ADMIN_LAB_ID}" \
  ...
  "$(command -v node)" scripts/deploy/test-admin-bigbang-js-budget.mjs "$1" "$2"
