...
if [[ $# -ne 1 || ! "$1" =~ ^sha256:[0-9a-f]{64}$ ]]; then
  echo 'usage: test-admin-bigbang-boundaries.sh sha256:<local-fixture-image-id>' >&2
  exit 2
fi
ADMIN_LAB_ID="admin-lab-boundaries-$$-$(date +%s)"
cleanup() {
  local id
...
systemd-run --user --quiet --wait --pipe --collect --unit="${ADMIN_LAB_ID}" \
  --property=RuntimeMaxSec=300 --property=KillMode=control-group --property=TimeoutStopSec=5 \
  --working-directory="${ADMIN_LAB_ROOT}" --setenv="ADMIN_FIXTURE_RUN_ID=${ADMIN_LAB_ID}" --setenv="PATH=${PATH}" \
  "$(command -v node)" scripts/deploy/test-admin-bigbang-boundaries.mjs "$1"
