...
grep -Fq 'BUILD_REVISION_SERVICES=(hololive-api hololive-alarm-worker youtube-collector holo-postgres deunhealth)' "${build_all}" \
    || fail "all-service build must verify application and rebuilt infrastructure revisions"
