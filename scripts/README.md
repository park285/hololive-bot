...
- `./scripts/ops/postgres-failover.sh` (+ `.service`/`.timer`, apply/env examples, `postgres-failover_test.sh`)
...
PostgreSQL controller의 checked-in unit은 dry-run입니다. 실제 승격은 승인된 external fencing과
