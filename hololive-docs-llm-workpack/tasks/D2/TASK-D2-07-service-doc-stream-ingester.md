# TASK-D2-07. 서비스 문서 추가: stream-ingester
...
`docs/current/services/stream-ingester.md`를 작성하여 `stream-ingester`의 현재 책임과 경계를 문서화합니다.

## 왜 필요한가

`stream-ingester`는 `Photo sync and ingestion-adjacent runtime` 역할을 갖습니다. 이 역할을 문서화해야 새 기능이 잘못된 서비스에 들어가는 일을 줄일 수 있습니다.
...
- `docs/current/services/stream-ingester.md`
- `docs/current/SERVICE_OWNERSHIP.md`
...
- `docs/current/services/stream-ingester.md`가 생성됩니다.
...
```text
Task TASK-D2-07만 수행하세요. 범위 밖 파일은 수정하지 마세요.
```
