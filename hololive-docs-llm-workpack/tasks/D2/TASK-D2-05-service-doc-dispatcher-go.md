# TASK-D2-05. 서비스 문서 추가: dispatcher-go
...
`docs/current/services/dispatcher-go.md`를 작성하여 `dispatcher-go`의 현재 책임과 경계를 문서화합니다.

## 왜 필요한가

`dispatcher-go`는 `Alarm dispatch queue consumer and Iris sender` 역할을 갖습니다. 이 역할을 문서화해야 새 기능이 잘못된 서비스에 들어가는 일을 줄일 수 있습니다.
...
- `docs/current/services/dispatcher-go.md`
- `docs/current/SERVICE_OWNERSHIP.md`
...
- `docs/current/services/dispatcher-go.md`가 생성됩니다.
...
```text
Task TASK-D2-05만 수행하세요. 범위 밖 파일은 수정하지 마세요.
```
