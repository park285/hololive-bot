...
값만 기록되기 때문이다(alarm_dispatch_deliveries.status의 TEXT+CHECK 선례와 동형). TEXT 전환은 098에서 kind 전 컬럼과
major_events.status까지 완료했고, 잔여 status·delivery_status·alarm_type 8컬럼은 varchar(20)+CHECK다(현행 vocab 최장
11자 — 폭은 여유 상한). 전면 TEXT 통일은 099 후속으로 트래킹한다: 해당 컬럼의 status partial index 11종이 ACCESS
EXCLUSIVE로 재빌드되므로 유지보수 창에서 수행한다. ② 사용자 입력이 닿는 표시명은 VARCHAR 폭을 통일한다 —
