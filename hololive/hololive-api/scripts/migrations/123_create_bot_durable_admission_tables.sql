...
-- state shape CHECK는 118 선례대로 단방향(status ⇒ 컬럼 존재)만 강제한다 —
-- 역방향까지 걸면 수동 회수가 lease 이력을 보존한 채 상태를 되돌리는 흐름이 막힌다.
...
CREATE TABLE IF NOT EXISTS bot_webhook_inbox (
    ...
    CONSTRAINT chk_bot_webhook_inbox_status_vocab CHECK (
        status IN ('pending', 'processing', 'retry', 'dead', 'succeeded', 'stale')
    ),
    CONSTRAINT chk_bot_webhook_inbox_message_id_size
        CHECK (length(message_id) > 0 AND length(message_id) <= 512),
    CONSTRAINT chk_bot_webhook_inbox_room_id_size CHECK (length(room_id) > 0),
    ...
    CONSTRAINT chk_bot_webhook_inbox_state_shape CHECK (
        ...
        AND (status NOT IN ('dead', 'stale') OR terminal_at IS NOT NULL)
    ),
    UNIQUE (message_id)
);
...
CREATE TABLE IF NOT EXISTS bot_reply_outbox (
    ...
    CONSTRAINT chk_bot_reply_outbox_room_id_size CHECK (length(room_id) > 0),
