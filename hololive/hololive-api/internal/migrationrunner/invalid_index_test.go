...
func TestInvalidIndexesDetectsAndDropsLeftoverInvalidIndex(t *testing.T) {
	...
	if dropErr := dropInvalidIndexes(ctx, conn, targets); dropErr == nil {
...
func TestInvalidIndexesExcludesInProgressConcurrentBuild(t *testing.T) {
	...
	snapshotTx, buildDone := startBlockedConcurrentIndexBuild(t, pool)
...
func startBlockedConcurrentIndexBuild(t *testing.T, pool *pgxpool.Pool) (pgx.Tx, <-chan error) {
	t.Helper()
	...
	// t.Context()는 Cleanup 직전에 취소되므로, rollback ctx는 취소에서 분리해야 한다.
	t.Cleanup(func() {
		rollbackErr := snapshotTx.Rollback(context.WithoutCancel(ctx))
		if rollbackErr != nil && !errors.Is(rollbackErr, pgx.ErrTxClosed) {
			t.Errorf("rollback snapshot tx: %v", rollbackErr)
		}
	})

	if _, execErr := snapshotTx.Exec(ctx, "SET TRANSACTION ISOLATION LEVEL REPEATABLE READ"); execErr != nil {
		t.Fatalf("set isolation: %v", execErr)
	}

	if _, execErr := snapshotTx.Exec(ctx, "SELECT count(*) FROM inv_idx_progress"); execErr != nil {
		t.Fatalf("hold snapshot: %v", execErr)
	}

	buildDone := make(chan error, 1)

	go func() {
		_, buildErr := pool.Exec(ctx, "CREATE INDEX CONCURRENTLY idx_inv_inprogress ON inv_idx_progress(x)")
		buildDone <- buildErr
	}()

	waitForCreateIndexProgress(t, pool, "idx_inv_inprogress")

	return snapshotTx, buildDone
}
...
func waitForCreateIndexProgress(t *testing.T, pool *pgxpool.Pool, indexName string) {
	t.Helper()
