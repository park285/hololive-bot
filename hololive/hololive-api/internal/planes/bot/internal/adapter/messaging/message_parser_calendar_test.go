...
func TestTryCelebrationCalendarCommand(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "달력 without args", command: "달력", args: nil, raw: "!달력", want: true},
		{name: "달력 with valid month", command: "달력", args: []string{"6"}, raw: "!달력 6", want: true, month: 6},
		{name: "달력 next month", command: "달력", args: []string{"다음달"}, raw: "!달력 다음달", want: true, offset: 1},
		{name: "달력 previous month", command: "달력", args: []string{"저번달"}, raw: "!달력 저번달", want: true, offset: -1},
		{name: "기념일 alias", command: "기념일", args: nil, raw: "!기념일", want: true},
		{name: "calendar alias", command: "calendar", args: nil, raw: "!calendar", want: true},
		{name: "invalid month ignored", command: "달력", args: []string{"13"}, raw: "!달력 13", want: true},
		{name: "non-numeric month ignored", command: "달력", args: []string{"abc"}, raw: "!달력 abc", want: true},
		{name: "unrelated command", command: "도움말", args: nil, raw: "!도움말", want: false},
	}
