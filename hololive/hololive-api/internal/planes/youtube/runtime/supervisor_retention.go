...
func (r *Runtime) retainSource(ctx context.Context) error {
	...
	err := r.withRetainDB(ctx, func(ctx context.Context) error {
		...
		if tickErr != nil {
			return fmt.Errorf("run retention tick: %w", tickErr)
		}
	...
	if err != nil {
		return fmt.Errorf("with retain DB: %w", err)
	}
...
func (r *Runtime) withRetainDB(ctx context.Context, fn func(context.Context) error) error {
	...
	if err := r.withDB(retainCtx, fn); err != nil {
...
func (r *Runtime) replayTick(ctx context.Context) error {
	...
	for range r.Config.Replay.BatchSize {
		processed, err := r.processNextReplay(ctx)
		if err != nil {
...
func (r *Runtime) processNextReplay(ctx context.Context) (bool, error) {
	var processed bool
	...
	err := r.withDB(ctx, func(ctx context.Context) error {
		...
		if tickErr != nil {
			return fmt.Errorf("process next replay: %w", tickErr)
		}
