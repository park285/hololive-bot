...
impl Drop for TracingRuntime {
    fn drop(&mut self) {
        ...
        {
            error!(error = %err, "failed to shutdown otel provider");
        }
    }
}
...
pub fn init_tracing(config: &LoggingConfig, telemetry: &TelemetryConfig) -> Result<TracingRuntime> {
    ...
    let stdout_timer = build_kst_timer().context("failed to initialize stdout KST timer")?;
    ...
    let file_timer = build_kst_timer().context("failed to initialize file KST timer")?;
...
pub fn resolve_telemetry_config(
    ...
    if cfg.service_name.trim().is_empty() {
        cfg.service_name = "hololive-scraper-rs".to_string();
    }
...
fn init_otel_runtime(config: &TelemetryConfig) -> Result<Option<OTelRuntime>> {
    ...
    let mut startup_span = tracer.start("scraper.startup");
    startup_span.end();
    if let Err(err) = provider.force_flush() {
        error!(error = %err, "failed to force_flush otel spans");
    }
...
{
    let kst_offset = time::UtcOffset::from_hms(KST_OFFSET_HOURS, 0, 0)
        .context("failed to initialize KST offset")?;
