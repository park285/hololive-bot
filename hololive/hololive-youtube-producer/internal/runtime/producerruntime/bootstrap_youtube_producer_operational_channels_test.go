...
func TestBuildYouTubeProducerRuntime_FailsWhenOperationalChannelSnapshotUnavailable(t *testing.T) {
	...
	cleanupCalls := 0
	initYouTubeProducerInfrastructureFn = func(context.Context, *settings.Config, *slog.Logger) (*youtubeProducerInfrastructure, error) {
		return &youtubeProducerInfrastructure{
			cleanup: func() { cleanupCalls++ },
		}, nil
	}

	runtime, err := BuildYouTubeProducerRuntime(context.Background(), &settings.Config{
		Server: settings.ServerConfig{Port: 30005},
