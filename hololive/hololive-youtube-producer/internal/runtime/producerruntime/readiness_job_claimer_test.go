...
func (s *readinessBudgetLimiterStub) MarkSourceCooldown(context.Context, polling.BudgetSource, time.Duration, string) error {
	s.markCalls++
	return s.markErr
}
...
func TestReadinessReportingJobClaimerMarksLeaseUnavailable(t *testing.T) {
	...
	if status.Result != polling.JobClaimUnavailable {
		t.Fatalf("status.Result = %s, want %s", status.Result, polling.JobClaimUnavailable)
	}
...
func TestReadinessReportingJobClaimerMarksLeaseAvailable(t *testing.T) {
	...
	claimer := newReadinessReportingJobClaimer(&readinessClaimStub{
		status: polling.JobClaimStatus{Result: polling.JobClaimPeerOwned},
	}, state)
	if claimer == nil {
		t.Fatal("newReadinessReportingJobClaimer returned nil")
	}
...
func TestReadinessReportingBudgetLimiterMarksBackendUnavailableOnError(t *testing.T) {
	...
	_, _, err := limiter.TryReserve(context.Background(), &polling.BudgetJob{PollerName: "videos"}, readinessBudgetProfile(polling.BudgetSourceYouTubeScraper), time.Minute)
...
func TestReadinessReportingBudgetLimiterMarksBudgetExhausted(t *testing.T) {
	...
	state.MarkRunning()
	limiter := newReadinessReportingBudgetLimiter(&readinessBudgetLimiterStub{
		decision: polling.BudgetDecision{Allowed: false, Reason: "budget_exhausted"},
	}, state)

	_, decision, err := limiter.TryReserve(context.Background(), &polling.BudgetJob{PollerName: "videos"}, readinessBudgetProfile(polling.BudgetSourceYouTubeScraper, polling.BudgetSourceHolodexLive), time.Minute)
...
func TestReadinessReportingBudgetLimiterMarkSourceCooldownUpdatesReadiness(t *testing.T) {
	...
	err := reporter.MarkSourceCooldown(context.Background(), polling.BudgetSourceYouTubeScraper, time.Minute, "youtube_rate_limited")
...
func TestReadinessReportingBudgetLimiterAllowedClearsAdmissionAndBackend(t *testing.T) {
	...
	_, decision, err := limiter.TryReserve(context.Background(), &polling.BudgetJob{PollerName: "videos"}, readinessBudgetProfile(polling.BudgetSourceYouTubeScraper), time.Minute)
...
func readinessBudgetProfile(sources ...polling.BudgetSource) polling.BudgetProfile {
	sourceUnits := make(map[polling.BudgetSource]float64, len(sources))
