...
func (c *recoveryLoopClaimer) TryClaim(
	...
	if len(c.responses) == 0 {
		return polling.JobClaimStatus{Result: polling.JobClaimUnavailable}, nil, fmt.Errorf("missing response")
	}
	index := call - 1
...
func TestRecoveryLoopIdleWhenLeaseAvailable(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		claimer := &recoveryLoopClaimer{
			responses: []recoveryLoopClaimResponse{
				{status: polling.JobClaimStatus{Result: polling.JobClaimAcquired}, claim: &recoveryLoopClaim{}},
			},
		}
...
func TestRecoveryLoopInvokesOnResumeOnTransition(t *testing.T) {
	...
	claimer := &recoveryLoopClaimer{
		responses: []recoveryLoopClaimResponse{
			{status: polling.JobClaimStatus{Result: polling.JobClaimUnavailable}, err: fmt.Errorf("valkey unavailable")},
			{status: polling.JobClaimStatus{Result: polling.JobClaimUnavailable}, err: fmt.Errorf("valkey unavailable")},
			{status: polling.JobClaimStatus{Result: polling.JobClaimAcquired}, claim: claim},
		},
	}
