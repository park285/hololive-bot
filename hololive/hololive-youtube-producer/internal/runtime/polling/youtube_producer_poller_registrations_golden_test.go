	...
	cases := []struct {
	...
	}{
		{
			...
			want: []string{
				...
				"name=live_batch | priority=2 | interval=2m0s | targetGroup=notification | channelIDs=[__global__] | hasExplicitChannelIDs=true | requestsPerRun=1 | worstCaseAttempts=1 | worstCaseRequestUnitsPerRun=3 | hasBudgetProfile=true | budgetBurstClass=primary | budgetPriority=high | budgetSourceUnits={holodex_live=1,postgres_write=3} | budgetFallbackSourceUnits={youtube_scraper=3}",
			},
		},
		{
