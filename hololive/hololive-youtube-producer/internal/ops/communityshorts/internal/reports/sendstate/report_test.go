...
func TestBuild(t *testing.T) {
	...
	require.Equal(t, PerPostStateNotSent, report.Rows[0].SendState)
	require.Equal(t, "short:post-attempted", report.Rows[1].ReportPostID)
	require.Equal(t, PerPostStateAttemptedWithoutSuccess, report.Rows[1].SendState)
	require.Equal(t, "community:post-sent", report.Rows[2].ReportPostID)
	require.Equal(t, PerPostStateSent, report.Rows[2].SendState)
