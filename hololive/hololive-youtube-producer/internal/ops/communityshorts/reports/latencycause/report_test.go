...
func TestBuild(t *testing.T) {
		...
		[]timeline.PostDeliveryTimeline{
			{
				...
				DelaySource:           timeline.PostDelaySourceExternalCollection,
				LatencyClassification: timeline.PostLatencyClassificationResult{
					...
					Evidence: []timeline.PostLatencyClassificationEvidence{{
						Key:      timeline.PostLatencyClassificationEvidenceKeyPublishToDetect,
			...
			},
			{
				...
				DelaySource:             timeline.PostDelaySourceInternalDelivery,
				InternalDelayCause:      timeline.PostInternalDelayCauseQueueWait,
				LatencyClassification: timeline.PostLatencyClassificationResult{
					Status:             timeline.PostLatencyClassificationStatusExceeded,
					ThresholdMillis:    int64(2 * time.Minute / time.Millisecond),
					DelaySource:        timeline.PostDelaySourceInternalDelivery,
					InternalDelayCause: timeline.PostInternalDelayCauseQueueWait,
					Evidence: []timeline.PostLatencyClassificationEvidence{{
						Key:      timeline.PostLatencyClassificationEvidenceKeyQueueWait,
	...
	require.Equal(t, timeline.PostDelaySourceInternalDelivery, lastHour.Rows[0].DelaySource)
	require.Equal(t, timeline.PostInternalDelayCauseQueueWait, lastHour.Rows[0].InternalDelayCause)
	...
	require.Equal(t, Evidence{
		...
		SelectedClassificationKeys: []timeline.PostLatencyClassificationEvidenceKey{
			timeline.PostLatencyClassificationEvidenceKeyQueueWait,
		},
	}, lastHour.Rows[0].CauseEvidence)
	require.Equal(t, timeline.PostLatencyClassificationStatusExceeded, lastHour.Rows[0].LatencyClassification.Status)
	...
	require.Equal(t, timeline.PostDelaySourceExternalCollection, lastHour.Rows[1].DelaySource)
	...
	require.Equal(t, Evidence{
		...
		SelectedClassificationKeys: []timeline.PostLatencyClassificationEvidenceKey{
			timeline.PostLatencyClassificationEvidenceKeyPublishToDetect,
		},
...
func TestBuild_UsesInsufficientEvidenceWhenTimelineMissing(t *testing.T) {
	...
	periods := []analytics.PostLatencyPeriod{{Label: "last_24h", StartAt: generatedAt.Add(-24 * time.Hour), EndAt: generatedAt}}
	...
	require.Equal(t, timeline.PostLatencyClassificationStatusInsufficientEvidence, report.Periods[0].Rows[0].LatencyClassification.Status)
...
func TestRenderMarkdown_OutputStability(t *testing.T) {
	...
	report := Report{
		...
		Periods: []PeriodView{{
			Summary: analytics.PostLatencyPeriodSummary{
				Label:                    "last_1h",
				StartAt:                  periodStart,
				EndAt:                    periodEnd,
				TotalPostCount:           1,
...
func TestBuild_ExternalCollectionOverridesInternalCauseForFailureAggregation(t *testing.T) {
	...
	periods := []analytics.PostLatencyPeriod{{Label: "last_1h", StartAt: generatedAt.Add(-time.Hour), EndAt: generatedAt}}
	...
	report, err := Build(
		...
		[]timeline.PostDeliveryTimeline{{
			...
			RetryAccumulationMillis: &retryAccumulationMillis,
			DelaySource:             timeline.PostDelaySourceExternalCollection,
			InternalDelayCause:      timeline.PostInternalDelayCauseRetryAccumulation,
			LatencyClassification: timeline.PostLatencyClassificationResult{
				Status:             timeline.PostLatencyClassificationStatusExceeded,
				ThresholdMillis:    int64(2 * time.Minute / time.Millisecond),
				DelaySource:        timeline.PostDelaySourceExternalCollection,
...
func TestBuildEvidence(t *testing.T) {
	...
	row := Row{
		DelaySource:        timeline.PostDelaySourceMixed,
		InternalDelayCause: timeline.PostInternalDelayCauseRetryAccumulation,
		LatencyClassification: timeline.PostLatencyClassificationResult{
			Status: timeline.PostLatencyClassificationStatusExceeded,
			Evidence: []timeline.PostLatencyClassificationEvidence{
				{Key: timeline.PostLatencyClassificationEvidenceKeyAlarmLatency, Selected: true},
				{Key: timeline.PostLatencyClassificationEvidenceKeyRetryAccumulation, Selected: true},
			},
		},
	}
	...
	require.Equal(t, Evidence{
		...
		SelectedClassificationKeys: []timeline.PostLatencyClassificationEvidenceKey{
			timeline.PostLatencyClassificationEvidenceKeyAlarmLatency,
			timeline.PostLatencyClassificationEvidenceKeyRetryAccumulation,
		},
	}, got)
}
...
func TestClassifyInternalJudgment(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			row: Row{
				DelaySource:           timeline.PostDelaySourceMixed,
				LatencyClassification: timeline.PostLatencyClassificationResult{Status: timeline.PostLatencyClassificationStatusExceeded},
			},
		...
		},
		{
			...
			row: Row{
				InternalDelayCause:    timeline.PostInternalDelayCauseJobFailure,
				LatencyClassification: timeline.PostLatencyClassificationResult{Status: timeline.PostLatencyClassificationStatusExceeded},
			},
			wantJudgment: InternalCauseJudgmentInternalSystem,
		...
		},
		{
			...
			row: Row{
				LatencyClassification: timeline.PostLatencyClassificationResult{Status: timeline.PostLatencyClassificationStatusInsufficientEvidence},
			},
			wantJudgment: InternalCauseJudgmentNonInternal,
...
func TestBuildPeriodReport(t *testing.T) {
	...
	report := BuildPeriodReport([]analytics.PostLatencyPeriodSummary{
		{
			Label:                      "last_24h",
