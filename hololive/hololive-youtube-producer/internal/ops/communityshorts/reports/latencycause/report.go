...
type Evidence struct {
	...
	SelectedClassificationKeys []timeline.PostLatencyClassificationEvidenceKey `json:"selected_classification_keys,omitempty"`
}
...
type PeriodView struct {
	Summary      analytics.PostLatencyPeriodSummary `json:"summary"`
...
type Row struct {
	...
	DelaySource             timeline.PostDelaySource                 `json:"delay_source"`
	InternalDelayCause      timeline.PostInternalDelayCause          `json:"internal_delay_cause"`
	...
	LatencyClassification   timeline.PostLatencyClassificationResult `json:"latency_classification"`
}

type rawRows struct {
	sendCountRows []analytics.PostSendCount
	timelineRows  []timeline.PostDeliveryTimeline
	query         Query
...
func normalizeCollectOptions(
	options CollectOptions,
	now time.Time,
) (Query, []analytics.PostLatencyPeriod, error) {
...
func earliestPeriodStart(periods []analytics.PostLatencyPeriod) time.Time {
	if len(periods) == 0 {
		return time.Time{}
	}
