...
func buildPeriodSummaryMarkdown(summary *analytics.PostLatencyPeriodSummary) string {
	if summary == nil {
		return ""
	}
...
func accumulateDelaySource(summary *Summary, delaySource timeline.PostDelaySource) {
	if delaySource == timeline.PostDelaySourceExternalCollection {
		summary.ExcludedExternalDelayPostCount++
		summary.ExternalCollectionSourcePostCount++

		return
	}
	if delaySource == timeline.PostDelaySourceInternalDelivery {
		summary.InternalDeliverySourcePostCount++

		return
	}
	if delaySource == timeline.PostDelaySourceMixed {
...
func incrementInternalCause(summary *Summary, cause timeline.PostInternalDelayCause) {
	if summary == nil {
		return
	}
	if cause == timeline.PostInternalDelayCauseQueueWait {
		summary.QueueWaitCausePostCount++

		return
	}
	if cause == timeline.PostInternalDelayCauseRetryAccumulation {
		summary.RetryAccumulationCausePostCount++

		return
	}
	if cause == timeline.PostInternalDelayCauseJobFailure {
