...
type PeriodReport struct {
	...
	Periods     []analytics.PostLatencyPeriodSummary `json:"periods"`
}
...
func BuildPeriodReport(
	rows []analytics.PostLatencyPeriodSummary,
	...
	normalizedRows := make([]analytics.PostLatencyPeriodSummary, 0, len(rows))
	for i := range rows {
...
func writePeriodRow(builder *strings.Builder, row *analytics.PostLatencyPeriodSummary) {
	if row == nil {
		return
	}
	builder.WriteString("| `")
...
func buildPeriods(
	now time.Time,
	specs []PeriodSpec,
) ([]analytics.PostLatencyPeriod, error) {
	now = shared.NormalizeSendCountTime(now)
	if now.IsZero() {
		now = time.Now().UTC()
	}
	...
	periods := make([]analytics.PostLatencyPeriod, 0, len(specs))
	...
	for i := range specs {
		...
		periods = append(periods, analytics.PostLatencyPeriod{
			Label:   label,
...
func clonePeriodSummary(row *analytics.PostLatencyPeriodSummary) analytics.PostLatencyPeriodSummary {
	if row == nil {
