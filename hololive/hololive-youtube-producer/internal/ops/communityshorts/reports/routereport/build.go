...
func Build(
	...
	pathUsageRows []analytics.PostDeliveryPathUsage,
...
func indexPathUsage(rows []analytics.PostDeliveryPathUsage) map[contentKey][]analytics.PostDeliveryPathUsage {
	index := make(map[contentKey][]analytics.PostDeliveryPathUsage)
	for i := range rows {
...
func indexSendCounts(rows []analytics.PostSendCount) map[routeKey][]analytics.PostSendCount {
	index := make(map[routeKey][]analytics.PostSendCount)
	for i := range rows {
...
func buildChannel(
	...
	sendCountIndex map[routeKey][]analytics.PostSendCount,
	pathUsageIndex map[contentKey][]analytics.PostDeliveryPathUsage,
	summary *Summary,
) Channel {
	if channel == nil {
		return Channel{}
...
func buildRoute(
	...
	sendCounts []analytics.PostSendCount,
	pathUsageIndex map[contentKey][]analytics.PostDeliveryPathUsage,
) Route {
...
func latestPublishedAt(current *time.Time, sendCount *analytics.PostSendCount) *time.Time {
	if sendCount == nil {
...
func latestSuccessAt(current *time.Time, sendCount *analytics.PostSendCount) *time.Time {
	if sendCount == nil {
		return shared.CloneSendCountTime(current)
	}
