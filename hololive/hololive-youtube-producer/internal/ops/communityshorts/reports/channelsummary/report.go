...
type Report struct {
	...
	Rows        []analytics.ChannelPostDeliverySummary `json:"rows"`
}
...
func Build(
	rows []analytics.ChannelPostDeliverySummary,
	...
	normalizedRows := make([]analytics.ChannelPostDeliverySummary, 0, len(rows))
	summary := Totals{}
	for i := range rows {
		row := rows[i]
...
func renderTableMarkdown(rows []analytics.ChannelPostDeliverySummary) string {
	var builder strings.Builder
...
func resolveStatus(row *analytics.ChannelPostDeliverySummary) string {
	if row == nil {
		return "ok"
	}
...
func sortTime(row *analytics.ChannelPostDeliverySummary) time.Time {
	if row == nil {
		return time.Time{}
	}
