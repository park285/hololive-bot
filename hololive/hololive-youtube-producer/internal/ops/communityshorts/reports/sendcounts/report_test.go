...
func TestBuild(t *testing.T) {
	...
	report := Build([]analytics.PostSendCount{
		{
	...
	require.Equal(t, VerificationStatusFail, report.Verification.DuplicateAlarmStatus)
	...
	byPostID := make(map[string]Row, len(report.Rows))
	...
	require.Equal(t, timeline.PostDelaySourceInternalDelivery, byPostID["post-duplicate"].DelaySource)
	require.Equal(t, timeline.PostInternalDelayCauseRetryAccumulation, byPostID["post-duplicate"].InternalDelayCause)
	...
	require.Equal(t, timeline.PostDelaySourceExternalCollection, byPostID["short-no-success"].DelaySource)
	require.Equal(t, timeline.PostInternalDelayCauseJobFailure, byPostID["short-no-success"].InternalDelayCause)
	...
	require.Equal(t, timeline.PostDelaySourceMixed, byPostID["short-ok"].DelaySource)
	require.Equal(t, timeline.PostInternalDelayCauseQueueWait, byPostID["short-ok"].InternalDelayCause)
...
func TestRenderMarkdown_OutputStability(t *testing.T) {
	...
	report := Report{
		...
		Verification: Verification{
			DuplicateAlarmStatus:    VerificationStatusPass,
			DuplicateAlarmPostCount: 0,
			DuplicateAlarmRule:      DuplicateAlarmRule,
		},
		Rows: []Row{{
			PostSendCount: analytics.PostSendCount{
				AlarmType:          domain.AlarmTypeCommunity,
				ChannelID:          "UC_TEST",
