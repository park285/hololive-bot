...
func CloneLatencyClassification(result *timeline.PostLatencyClassificationResult) timeline.PostLatencyClassificationResult {
	if result == nil {
		return timeline.PostLatencyClassificationResult{}
	}
	cloned := timeline.PostLatencyClassificationResult{
	...
	cloned.Evidence = make([]timeline.PostLatencyClassificationEvidence, 0, len(result.Evidence))
	for i := range result.Evidence {
		item := timeline.PostLatencyClassificationEvidence{
...
func RenderLatencyClassificationEvidence(result *timeline.PostLatencyClassificationResult) string {
	if result == nil {
		return NoneValue
	}
	if len(result.Evidence) == 0 {
...
func FormatLatencyClassificationEvidenceItem(item timeline.PostLatencyClassificationEvidence) string {
	value := NoneValue
	if item.Millis != nil {
