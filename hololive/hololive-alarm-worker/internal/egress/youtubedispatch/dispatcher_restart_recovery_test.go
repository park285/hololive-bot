...
func TestProcessOnce_RetriesPersistedDeliveriesWithoutNewOutboxClaim(t *testing.T) {
	...
	dispatcher := NewDispatcher(db, cachemocks.NewLenientClient(), sender, nil, slog.New(slog.NewTextHandler(io.Discard, nil)), &dispatchstate.Config{
		BatchSize:           10,
...
func TestProcessOnce_DoesNotResendAlreadySentCommunityShortsPostAfterDispatcherRestart(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			firstDispatcher := NewDispatcher(db, cachemocks.NewLenientClient(), firstSender, nil, slog.New(slog.NewTextHandler(io.Discard, nil)), &dispatchstate.Config{
				BatchSize:           10,
				LockTimeout:         time.Minute,
				PollInterval:        time.Second,
				MaxRetries:          3,
			...
			secondDispatcher := NewDispatcher(db, cachemocks.NewLenientClient(), secondSender, nil, slog.New(slog.NewTextHandler(io.Discard, nil)), &dispatchstate.Config{
				BatchSize:           10,
				LockTimeout:         time.Minute,
				PollInterval:        time.Second,
				MaxRetries:          3,
			...
			require.NoError(t, firstDeliveryTestRow(db, &persistedDelivery, delivery.ID).Error)
			...
			require.NoError(t, firstDeliveryTestRow(db, &persistedOutbox, item.ID).Error)
			...
			require.NoError(t, firstDeliveryTestRowWhere(db, &persistedTracking, "kind = ? AND content_id = ?", string(item.Kind), item.ContentID).Error)
			...
			require.NoError(t, firstDeliveryTestRow(db, &persistedState, "kind = ? AND post_id = ?", item.Kind, postID).Error)
