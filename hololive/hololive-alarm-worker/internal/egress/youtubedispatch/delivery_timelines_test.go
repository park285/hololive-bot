...
func TestDeliveryTelemetryRepository_ListPostDeliveryTimelinesSince_BuildsLatencyTimelineMetrics(t *testing.T) {
	...
	require.True(t, evidenceByKey[timeline.PostLatencyClassificationEvidenceKeyInternalLatency].Selected)
	require.NotNil(t, evidenceByKey[timeline.PostLatencyClassificationEvidenceKeyRetryAccumulation].Millis)
	require.Equal(t, int64(80*time.Second/time.Millisecond), *evidenceByKey[timeline.PostLatencyClassificationEvidenceKeyRetryAccumulation].Millis)
	require.True(t, evidenceByKey[timeline.PostLatencyClassificationEvidenceKeyRetryAccumulation].Selected)
}
...
func TestDerivePostDeliveryTimelineMetrics_ClassifiesExternalCollectionDelaySource(t *testing.T) {
	...
	require.Equal(t, timeline.PostDelaySourceExternalCollection, row.DelaySource)
	require.Equal(t, timeline.PostLatencyClassificationStatusExceeded, row.LatencyClassification.Status)
	...
	require.Equal(t, int64(100*time.Second/time.Millisecond), *evidenceByKey[timeline.PostLatencyClassificationEvidenceKeyPublishToDetect].Millis)
}
...
func TestDerivePostDeliveryTimelineMetrics_ClassifiesMixedDelaySource(t *testing.T) {
	...
	require.NotNil(t, row.InternalLatencyExceeded)
	require.False(t, *row.InternalLatencyExceeded)
	require.Equal(t, timeline.PostDelaySourceMixed, row.DelaySource)
}
...
func TestDerivePostDeliveryTimelineMetrics_ClassifiesQueueWaitPrimary(t *testing.T) {
	...
	require.False(t, row.JobFailureDetected)
	require.Equal(t, timeline.PostDelaySourceNone, row.DelaySource)
	require.Equal(t, timeline.PostInternalDelayCauseQueueWait, row.InternalDelayCause)
}
...
func TestDerivePostDeliveryTimelineMetrics_ClassifiesJobFailurePrimary(t *testing.T) {
	...
	require.Equal(t, timeline.PostDelaySourceNone, row.DelaySource)
	require.Equal(t, timeline.PostInternalDelayCauseJobFailure, row.InternalDelayCause)
}

func indexPostLatencyClassificationEvidence(items []timeline.PostLatencyClassificationEvidence) map[timeline.PostLatencyClassificationEvidenceKey]timeline.PostLatencyClassificationEvidence {
	indexed := make(map[timeline.PostLatencyClassificationEvidenceKey]timeline.PostLatencyClassificationEvidence, len(items))
	for i := range items {
