...
func TestSendEngineKaringMutexWaitUsesDeliverySendTimeout(t *testing.T) {
	sender := &youtubeOutboxKaringTestSender{}
	engine := newSendEngine(sender, &MessageFormatter{}, slog.New(slog.NewTextHandler(io.Discard, nil)), &dispatchstate.Config{
		DeliverySendTimeout: 20 * time.Millisecond,
	}, nil, newAuditLogger(nil, nil, slog.New(slog.NewTextHandler(io.Discard, nil)), &dispatchstate.Config{}, nil), nil)
