...
func TestEnqueueDeliveries_NoSubscribersMarksShortSentAtWithCanonicalTimestamp(t *testing.T) {
	...
	item := domain.YouTubeNotificationOutbox{
		...
		Payload:       `{"canonical_post_id":"short:short-no-subscribers","video_id":"short-no-subscribers","title":"short-title"}`,
...
func TestDeliveryRepositoryStoresShortPublishedAtAndSentAtWithCanonicalTimestamp(t *testing.T) {
	...
	item := domain.YouTubeNotificationOutbox{
		...
		Payload:       `{"canonical_post_id":"short:short-delivery","video_id":"short-delivery","title":"short-title","published_at":"2026-04-10T01:10:00Z"}`,
		Status:        domain.OutboxStatusPending,
		AttemptCount:  0,
		NextAttemptAt: nextAttemptAt,
	}
...
func TestDeliveryRepositoryMarkSentBatchRecordsCommunityAlarmSentAtWithCanonicalTimestamp(t *testing.T) {
	...
	item := domain.YouTubeNotificationOutbox{
		...
		Payload:       `{"canonical_post_id":"community:post-tracking","post_id":"post-tracking","content_text":"community-title","published_at":"2026-04-10T01:09:00Z"}`,
		Status:        domain.OutboxStatusPending,
		AttemptCount:  0,
	...
	require.Equal(t, string(timeline.PostLatencyClassificationStatusExceeded), updatedTracking.LatencyClassificationStatus)
	require.Equal(t, string(timeline.PostDelaySourceInternalDelivery), updatedTracking.DelaySource)
	require.Equal(t, string(timeline.PostInternalDelayCauseQueueWait), updatedTracking.InternalDelayCause)
}
...
func seedDuplicateExecutionSentAtRows(t *testing.T, db *deliveryTestDB) (
	...
	item := domain.YouTubeNotificationOutbox{
		...
		Payload:       `{"canonical_post_id":"short:short-tracking","video_id":"short-tracking","title":"short-title","published_at":"2026-04-10T01:09:00Z"}`,
		Status:        domain.OutboxStatusPending,
		AttemptCount:  0,
		NextAttemptAt: detectedAt,
	}
