...
SELECT
  ...
  COUNT(id) FILTER (WHERE status = 'quarantined'),
  COALESCE(MAX(EXTRACT(EPOCH FROM (NOW() - quarantined_at))) FILTER (WHERE status = 'quarantined'), 0)
FROM alarm_dispatch_deliveries
WHERE status IN ('pending', 'retry', 'sending', 'quarantined')
