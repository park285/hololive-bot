...
func (r *Runner) scheduleFrom(now, lastScheduledAt time.Time) time.Time {
...
func (r *Runner) logRunFailure(err error) {
...
func (r *Runner) findCelebrationCandidates(ctx context.Context, month, day, year int) (celebrationCandidates, error) {
...
func (r *Runner) effectiveInterval() time.Duration {
	if r.runInterval > 0 {
...
func (r *Runner) effectiveNow() time.Time {
	if r.now != nil {
		return r.now()
	}
	return util.NowKST()
