...
3. 현재 서버의 `/api/holo/*` 는 기본적으로 `X-API-Key` 기반입니다. 단, `GET /api/holo/streams/live`, `GET /api/holo/streams/upcoming` 는 공개 조회 엔드포인트입니다. 앱이 그 외 `/api/holo/*` 를 직접 호출해야 한다면 **API Key를 앱에 포함하는 구조는 보안상 위험**합니다. (권장: 보호된 `/api/holo/*` 를 세션 기반으로 전환하거나, 서버 내부 프록시를 둠)
