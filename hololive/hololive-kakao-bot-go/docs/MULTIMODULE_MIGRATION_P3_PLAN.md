...
> **상태 업데이트 (2026-02-28)**: P3-1 ~ P3-6 구현/검증 완료. 현재 운영 구조는 `hololive-bot` + `hololive-alarm` + `hololive-admin` + `hololive-llm-sched` + `hololive-stream-ingester` + `hololive-shared` 다중 모듈 기준.
...
| `hololive-kakao-bot-go/Dockerfile.stream-ingester` | `hololive-stream-ingester/Dockerfile` |
...
└── hololive-kakao-bot-go/           # 아직 5개 바이너리가 한 모듈에 공존
    ...
    │   ├── stream-ingester/         # 포트 없음 (cron worker)
...
| `settings_applier_local.go` | `shared/pkg/server/` | bot + stream-ingester 사용 |
...
## P3-4: stream-ingester 독립 모듈

### 목표

```
llm/hololive-stream-ingester/
├── go.mod
...
```
1. llm/hololive-stream-ingester/ 디렉토리 + go.mod 생성
...
`bootstrap_stream_ingester.go` import:
...
```bash
go build ./hololive-stream-ingester/...
```
...
2. 불필요해진 파일 삭제:
   - Dockerfile.alarm-dispatcher, Dockerfile.llm-scheduler, Dockerfile.stream-ingester, Dockerfile.admin-api
   - cmd/alarm-dispatcher/, cmd/admin-api/, cmd/llm-scheduler/, cmd/stream-ingester/
...
├── hololive-stream-ingester/       # stream-ingester 전용
├── game-bot-go/
├── mcp-llm-server-go/
└── admin-dashboard/backend/
```
...
P3-1 (server/ 분할)
  ...
  │
  ├──→ P3-4 (stream-ingester)      # server/ 비의존, 병렬 가능
  │
...
### P3-4: stream-ingester 분리
- [x] `llm/hololive-stream-ingester/` 디렉토리 + `go.mod` 생성
- [x] `go.work`에 `./hololive-stream-ingester` 추가
- [x] `cmd/stream-ingester/` 이동
- [x] `bootstrap_stream_ingester.go` 이동 (이미 providers.* 직접 참조)
- [x] Dockerfile 빌드 컨텍스트 변경
- [x] `go build ./hololive-stream-ingester/...` 통과
- [x] bot-go에서 stream-ingester 관련 코드 삭제
...
- [x] 불필요 Dockerfile 삭제 (alarm-dispatcher, admin-api, llm-scheduler, stream-ingester)
