...
> 마지막 업데이트: 2026-02-28
> 대상 서비스: `llm-scheduler` (포트 `30003`), `hololive-admin-api` 운영 API (포트 `30006`)
...
docker logs --tail 150 hololive-admin-api
```
...
### B. `hololive-admin-api` 운영 API 트리거 호출 실패(5xx/timeout)
1. `hololive-admin-api` 로그에서 upstream(`llm-scheduler`) 오류 확인
...
```bash
curl -sS -X POST "http://127.0.0.1:30006/api/holo/majorevent/trigger" \
  -H "X-API-Key: ${API_KEY}" \
...
```bash
curl -sS -X POST "http://127.0.0.1:30006/api/holo/majorevent/monthly-trigger" \
  -H "X-API-Key: ${API_KEY}" \
  -H "Content-Type: application/json"
```
...
```bash
curl -sS -X POST "http://127.0.0.1:30006/api/holo/settings/llm" \
  -H "X-API-Key: ${API_KEY}" \
  -H "Content-Type: application/json" \
