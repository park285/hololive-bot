...
└── hololive-kakao-bot-go/        # 381 tests PASS, 27 packages
    ├── cmd/{bot,alarm-dispatcher,admin-api,llm-scheduler,youtube-producer,tools}/
    └── internal/
...
| `ingestion_lock.go` | youtube-producer | youtube-producer 단독 ingestion ownership 보호 |
...
| `bootstrap_youtube_producer.go` | hololive-youtube-producer | youtube-producer 전용 |
...
| `settings_applier_local.go` | shared | bot + youtube-producer |
...
3. `api_router.go`, `iris_sender_adapter.go` → shared 이동, `ingestion_lock.go` → youtube-producer runtime 이동
...
### P3-4: hololive-youtube-producer 모듈
```
