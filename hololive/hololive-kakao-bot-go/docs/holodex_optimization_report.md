...
> - live-status fallback은 채널별 **YouTube scraper**만 사용하며, 이 경로에서 **공식 스케줄 페이지 재조회는 하지 않습니다**.
...
- **Fallback 경로**: 채널별 **YouTube scraper** 조회만 허용
...
graph TD
    ...
    C -- retryable 실패 --> E[채널별 YouTube scraper 폴백]
...
| `pkg/service/holodex/scraper.go` | channel schedule에서 YouTube scraper 우선, 공식 스케줄 fallback 조건 축소 |
