...
| P6 | Stream Ingester 분리 | ✅ 완료 | stream-ingester 바이너리 신설 + bot ingestion 코드 제거 + 분산 limiter/ingestion lock 적용 |
...
- ingestion 분산 락 보강
  - `hololive-shared/pkg/providers/ingestion_lock.go` (SetNX + compare-and-expire renew + CompareAndDelete release)
  - `internal/service/cache/service.go` (`CompareAndExpire` CAS helper)
  - `hololive-shared/pkg/providers/ingestion_lock_test.go` / `internal/service/cache/compare_and_expire_test.go`
- 운영 Runbook
