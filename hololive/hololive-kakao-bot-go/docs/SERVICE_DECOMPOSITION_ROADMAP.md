...
| P6 | Stream Ingester 분리 | ✅ 완료 | stream-ingester 바이너리 신설 + Bot ingestion 토글 분리 + 분산 limiter/ingestion lock 적용 |
...
└──────────────────────────────────────────────────────┘
              ↓ alarm:dispatch:queue (JSON v1)
┌─ Go ────────────────────────────────────────────────┐
│                                                      │
│  alarm-dispatcher (P3)                               │
│    BRPOP → 렌더 → Iris 발송                           │
│    Alarm CRUD API (/internal/alarm/*)                │
│    DB: alarms, Valkey: alarm:*, notified:*           │
│                                                      │
│  hololive-bot (축소)                                  │
│    Iris 웹훅 → 커맨드 라우팅 → 응답                    │
│    Alarm → alarm-dispatcher HTTP 클라이언트            │
│                                                      │
│  admin-api (P4)                                      │
...
| `internal/app/providers.go` | ProvideAlarmQueueDispatcher→domain.AlarmDispatchState |
...
| `internal/app/bootstrap_llm.go` | `buildMajorEventComponents` → `majorEventCfg config.MajorEventConfig` |
...
- `majorevent_scrape_hour_kst` (`{"hour_kst": N}`) → 스크래퍼 스케줄 시간 반영
- `majorevent_scrape_run_now` (`{}`) → 즉시 스크래핑 실행
...
- YouTube scraper 경로에 분산 레이트 리미터 연결
  ...
  - `internal/constants/constants.go` (`YouTubeScraperDistributedRateLimitConfig`)
  - `internal/service/youtube/scraper/distributed_rate_limit_test.go`
- 운영/설계 문서
  - `docs/DISTRIBUTED_RATE_LIMITING.md`
- stream-ingester 런타임 분리 (1차)
  - `cmd/stream-ingester/main.go`
  - `internal/app/bootstrap_stream_ingester.go`
  - `Dockerfile.stream-ingester`
  - `docker-compose.prod.yml` (`stream-ingester` 서비스 + bot `BOT_INGESTION_ENABLED=false`)
  - `internal/config/config.go` (`BOT_INGESTION_ENABLED` 토글)
  - `internal/app/bootstrap_runtime.go` / `internal/app/runtime.go` (bot ingestion 토글 반영)
- ingestion 분산 락 보강
  - `internal/app/ingestion_lock.go` (SetNX + compare-and-expire renew + CompareAndDelete release)
  - `internal/service/cache/service.go` (`CompareAndExpire` CAS helper)
  - `internal/app/ingestion_lock_test.go` / `internal/service/cache/compare_and_expire_test.go`
- 운영 Runbook
...
|-----------------|-----------|----------|
| `AlarmQueueDispatcher` | P3 | alarm-dispatcher |
...
| `Scheduler/ScraperScheduler/PhotoSync/OutboxDispatcher` | P6 ✅ | stream-ingester |
...
| 분산 rate limiter (P6) | P6 | 중간 | 중간 | Valkey sliding window(holodex + youtube scraper 경로 적용) + stream-ingester 경로 확장 |
| Bot/stream-ingester 동시 ingestion 실행 | P6 | 중간 | 높음 | `BOT_INGESTION_ENABLED` 토글 강제 + compose 기본값(bot=false, stream-ingester=true) + 분산 락(`lock:ingestion:runtime`, compare-and-expire renew) |
...
- stream-ingester 분리 운영 점검 (`docs/STREAM_INGESTER_RUNBOOK.md`)
