...
- `hololive-bot`: `BOT_INGESTION_ENABLED=false`
- `stream-ingester`: `BOT_INGESTION_ENABLED=true`, `SERVER_PORT=30004`
...
2. 환경 변수 확인
   - `hololive-bot`: `BOT_INGESTION_ENABLED=false`
   - `stream-ingester`: `BOT_INGESTION_ENABLED=true`
3. 로그 확인
   - bot에서 `event=bot_ingestion_enabled`가 **없어야 함**
...
docker logs --since 15m hololive-bot | grep "bot_ingestion_enabled"
```

## 6) 롤백 절차 (비상)

1. `stream-ingester` 중지
2. `hololive-bot`에 `BOT_INGESTION_ENABLED=true` 적용 후 재기동
3. `stream-ingester`는 `BOT_INGESTION_ENABLED=false` 또는 중지 상태 유지
4. `event=ingestion_lease_acquired`, `role=bot` 확인

```bash
docker compose -f docker-compose.prod.yml stop stream-ingester
docker compose -f docker-compose.prod.yml up -d --build hololive-bot
docker logs --since 10m hololive-bot | grep "ingestion_lease_acquired"
```
