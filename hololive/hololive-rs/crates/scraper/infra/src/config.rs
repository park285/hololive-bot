...
impl AppConfig {
    ...
    pub fn load_from_path(path: impl AsRef<Path>) -> Result<Self, ConfigLoadError> {
        let settings = config::Config::builder()
            ...
            .set_default("telemetry.service_name", "hololive-rs")?
...
mod tests {
    ...
    fn load_config_applies_scraper_env_overrides() {
        ...
        unsafe {
            ...
            std::env::set_var("SCRAPER__TELEMETRY__SERVICE_NAME", "hololive-rs-test");
        ...
        assert_eq!(loaded.telemetry.service_name, "hololive-rs-test");
    ...
    #[test]
    fn telemetry_otlp_insecure_default_is_false() {
        let _guard = ENV_LOCK.lock().expect("env lock should be available");
        let env_guard = EnvVarGuard::new(vec!["SCRAPER__TELEMETRY__OTLP_INSECURE"]);

        let temp_config = std::env::temp_dir().join(format!(
            "hololive-scraper-config-default-{}.toml",
            std::process::id()
        ));
        fs::write(&temp_config, "").expect("temporary config should be writable");
