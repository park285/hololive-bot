use alarm_core::{constants::FULL_REFRESH_INTERVAL, model::Stream};
...
use self::state::ChannelScheduleState;
...
impl TieredScheduler {
    ...
    pub fn compute_next_check_at(
        ...
        policies::compute_next_check_at(nearest_start, last_notified_at)
    }
    ...
    pub fn update_channel_state(&self, channel_id: &str, streams: &[Stream]) {
        ...
        self.states
            ...
            .and_modify(|st| {
                st.set_last_checked_at(now);
                st.set_next_check_at(next_at);
                st.set_last_streams_count(streams.len());
                st.set_force_due(false);
                st.set_nearest_start_at(nearest);
            })
            .or_insert_with(|| {
                ChannelScheduleState::new(
                    next_at,
                    now,
                    nearest,
                    false,
    ...
    pub fn mark_channel_due(&self, channel_id: &str) {
        ...
        self.states
            .entry(channel_id.to_owned())
            .and_modify(|st| {
                st.set_force_due(true);
                st.set_next_check_at(now);
            })
            .or_insert_with(|| {
                ChannelScheduleState::new(now, DateTime::UNIX_EPOCH, None, true, 0, None)
            });
    ...
    pub fn mark_channel_recently_notified(&self, channel_id: &str) {
        ...
        self.states
            .entry(channel_id.to_owned())
            .and_modify(|st| {
                st.set_last_notified_at(Some(now));
            })
            .or_insert_with(|| {
                ChannelScheduleState::new(now, DateTime::UNIX_EPOCH, None, false, 0, Some(now))
            });
    }
