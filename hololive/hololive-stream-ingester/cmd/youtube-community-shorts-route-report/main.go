...
func main() {
	...
	report, err := app.CollectCommunityShortsRouteVerificationReport(ctx, cfg, logger, now, now.Add(-*window))
	if err != nil {
		fmt.Fprintf(os.Stderr, "Failed to collect community/shorts route verification report: %v\n", err)
		os.Exit(1)
	}

	if _, err := fmt.Print(app.RenderCommunityShortsRouteVerificationMarkdown(report)); err != nil {
