...
func main() {
	...
	report, err := opsapp.CollectCommunityShortsChannelSummaryReport(ctx, cfg, logger, now, now.Add(-*window))
	if err != nil {
	...
	case "markdown":
		if _, err := fmt.Print(opsapp.RenderCommunityShortsChannelSummaryMarkdown(report)); err != nil {
