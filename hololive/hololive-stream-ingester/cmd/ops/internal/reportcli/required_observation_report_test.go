...
func TestRunRequiredObservationReport(t *testing.T) {
	...
	t.Run("rejects incomplete observation query", func(t *testing.T) {
		...
		err := RunRequiredObservationReport(
			RequiredObservationParams{Runtime: "youtube-scraper", Cutover: "", Format: "markdown"},
			newCommand(&stdout),
		)
	...
	t.Run("wraps config load errors", func(t *testing.T) {
		...
		err := RunRequiredObservationReport(
			RequiredObservationParams{Runtime: "youtube-scraper", Cutover: "2026-04-10T00:00:00Z", Format: "markdown"},
			command,
		)
	...
	t.Run("writes markdown output", func(t *testing.T) {
		...
		if stdout.String() != "markdown:youtube-scraper@2026-04-10T00:00:00Z" {
	...
	t.Run("writes json output", func(t *testing.T) {
		var stdout bytes.Buffer
		err := RunRequiredObservationReport(
			RequiredObservationParams{Runtime: "youtube-scraper", Cutover: "2026-04-10T00:00:00Z", Format: "json"},
			newCommand(&stdout),
		)
		if err != nil {
			t.Fatalf("RunRequiredObservationReport() error = %v", err)
	...
	t.Run("rejects unsupported format", func(t *testing.T) {
		var stdout bytes.Buffer
		err := RunRequiredObservationReport(
			RequiredObservationParams{Runtime: "youtube-scraper", Cutover: "2026-04-10T00:00:00Z", Format: "yaml"},
			newCommand(&stdout),
		)
