...
func TestValidateCommunityAlarmSentHistoryCLIArgs(t *testing.T) {
	t.Parallel()

	t.Run("accepts complete observation key", func(t *testing.T) {
		err := validateCommunityAlarmSentHistoryCLIArgs("youtube-scraper", "2026-04-10T00:00:00Z")
		if err != nil {
			t.Fatalf("validateCommunityAlarmSentHistoryCLIArgs() error = %v", err)
		}
	})

	t.Run("rejects missing observation key", func(t *testing.T) {
		err := validateCommunityAlarmSentHistoryCLIArgs("", "")
	...
	t.Run("rejects partial observation key", func(t *testing.T) {
		err := validateCommunityAlarmSentHistoryCLIArgs("youtube-scraper", "")
