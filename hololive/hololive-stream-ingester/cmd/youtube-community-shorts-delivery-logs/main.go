...
func main() {
	...
	options := app.CommunityShortsDeliveryLogCollectOptions{
	...
	report, err := app.CollectCommunityShortsDeliveryLogReport(ctx, cfg, logger, now, options)
	if err != nil {
	...
	case "markdown":
		if _, err := fmt.Print(app.RenderCommunityShortsDeliveryLogMarkdown(report)); err != nil {
