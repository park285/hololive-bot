...
func RenderCommunityShortsAlarmSentHistoryDatasetMarkdown(report CommunityShortsAlarmSentHistoryDatasetReport) string {
	...
	fmt.Fprintf(&builder, "%d", report.Summary.CollectedRowCount)
	builder.WriteString("`, duplicates_removed=`")
	fmt.Fprintf(&builder, "%d", report.Summary.DuplicateRowCount)
	builder.WriteString("`, sent_posts=`")
	fmt.Fprintf(&builder, "%d", report.Summary.SentPostCount)
	builder.WriteString("`, community_posts=`")
	fmt.Fprintf(&builder, "%d", report.Summary.CommunitySentPostCount)
	builder.WriteString("`, shorts_posts=`")
	fmt.Fprintf(&builder, "%d", report.Summary.ShortsSentPostCount)
	builder.WriteString("`, baseline_posts=`")
	fmt.Fprintf(&builder, "%d", report.Summary.BaselinePostCount)
	builder.WriteString("`, matched_posts=`")
	fmt.Fprintf(&builder, "%d", report.Summary.MatchedPostCount)
	builder.WriteString("`, unsent_posts=`")
	fmt.Fprintf(&builder, "%d", report.Summary.UnsentPostCount)
	builder.WriteString("`, duplicate_sent_posts=`")
	fmt.Fprintf(&builder, "%d", report.Summary.DuplicateSentPostCount)
	builder.WriteString("`, unexpected_sent_posts=`")
	fmt.Fprintf(&builder, "%d", report.Summary.UnexpectedSentPostCount)
	builder.WriteString("`, identifier_mismatch_candidates=`")
	fmt.Fprintf(&builder, "%d", report.Summary.IdentifierMismatchCandidateCount)
	builder.WriteString("`, verification_rows=`")
	fmt.Fprintf(&builder, "%d", report.Summary.VerificationRowCount)
	builder.WriteString("`, reference_rows=`")
	fmt.Fprintf(&builder, "%d", report.Summary.ReferenceRowCount)
	builder.WriteString("`, send_state_posts=`")
	fmt.Fprintf(&builder, "%d", report.Summary.SendStatePostCount)
	builder.WriteString("`, missing_alarm_posts=`")
	fmt.Fprintf(&builder, "%d", report.Summary.MissingAlarmPostCount)
	builder.WriteString("`, missing_send_state_posts=`")
	fmt.Fprintf(&builder, "%d", report.Summary.MissingSendStatePostCount)
	builder.WriteString("`, attempted_missing_posts=`")
	fmt.Fprintf(&builder, "%d", report.Summary.AttemptedMissingPostCount)
	builder.WriteString("`, not_sent_missing_posts=`")
	fmt.Fprintf(&builder, "%d", report.Summary.NotSentMissingPostCount)
	...
	if report.Results.MissingAlarmEvaluated {
		...
		fmt.Fprintf(&builder, "%d", report.Results.MissingAlarmPostCount)
		builder.WriteString("`, missing_send_state_posts=`")
		fmt.Fprintf(&builder, "%d", report.Results.MissingSendStatePostCount)
		builder.WriteString("`, attempted_missing_posts=`")
		fmt.Fprintf(&builder, "%d", report.Results.AttemptedMissingPostCount)
		builder.WriteString("`, not_sent_missing_posts=`")
		fmt.Fprintf(&builder, "%d", report.Results.NotSentMissingPostCount)
		builder.WriteString("`\n")
	...
	if len(report.Results.AlarmTypeComparisons) == 0 {
		...
		for i := range report.Results.AlarmTypeComparisons {
			...
			builder.WriteString("` | ")
			fmt.Fprintf(&builder, "%d", row.BaselinePostCount)
			builder.WriteString(" | ")
			fmt.Fprintf(&builder, "%d", row.SentPostCount)
			builder.WriteString(" | ")
			fmt.Fprintf(&builder, "%d", row.MatchedPostCount)
			builder.WriteString(" | ")
			fmt.Fprintf(&builder, "%d", row.UnsentPostCount)
			builder.WriteString(" | ")
			fmt.Fprintf(&builder, "%d", row.DuplicateSentPostCount)
			builder.WriteString(" | ")
			fmt.Fprintf(&builder, "%d", row.UnexpectedSentPostCount)
			builder.WriteString(" | ")
			fmt.Fprintf(&builder, "%d", row.IdentifierMismatchCandidateCount)
			builder.WriteString(" | ")
			fmt.Fprintf(&builder, "%d", row.MissingAlarmPostCount)
			builder.WriteString(" |\n")
		}
	}
