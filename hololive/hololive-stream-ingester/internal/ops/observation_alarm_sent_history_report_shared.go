...
func renderObservationAlarmSentHistoryMarkdown(
	...
	builder.WriteString(fmt.Sprintf("%d", summary.CollectedRowCount))
	builder.WriteString("`, duplicates_removed=`")
	builder.WriteString(fmt.Sprintf("%d", summary.DuplicateRowCount))
	builder.WriteString("`, sent_posts=`")
	builder.WriteString(fmt.Sprintf("%d", summary.SentPostCount))
