...
func TestBuildObservationWindow(t *testing.T) {
	...
	record, err := BuildObservationWindow(RuntimeOwnerYouTubeScraper, "2.0.0", policy, deploymentCompletedAt)
	require.NoError(t, err)
	require.Equal(t, RuntimeOwnerYouTubeScraper, record.RuntimeName)
