...
type CommunityShortsContinuousObservationReport struct {
	...
	TargetBaseline              CommunityShortsTargetBaseline                                `json:"target_baseline"`
...
func CollectCommunityShortsContinuousObservationReport(
	...
	targetBaseline, err := CollectCommunityShortsTargetBaseline(ctx, cfg, logger)
	if err != nil {
...
func renderCommunityShortsContinuousObservationTargetBaseline(
	...
	for i := range baseline.Channels {
		communityRoute, _ := communityShortsRouteForType(baseline.Channels[i].Routes, domain.AlarmTypeCommunity)
		shortsRoute, _ := communityShortsRouteForType(baseline.Channels[i].Routes, domain.AlarmTypeShorts)
		builder.WriteString("| `")
