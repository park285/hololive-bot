...
func TestHTTP009GzipBombHitsDecompressedCap(t *testing.T) {
	...
	_, err := ReadProviderJSONDocument(t.Context(), resp, DefaultJSONPolicy(1024), contract.ProviderHolodex)
...
func readTestProviderResponse(t *testing.T, client *http.Client, endpoint string) error {
	...
	if _, err = ReadProviderJSONDocument(t.Context(), resp, DefaultJSONPolicy(2048), contract.ProviderHolodex); err != nil {
		return fmt.Errorf("read provider JSON document: %w", err)
	}
...
func readStatus(t *testing.T, status int, retryAfter, body string) error {
	...
	if err := readResponse(t, resp, DefaultJSONPolicy(1024)); err != nil {
...
func readResponse(t *testing.T, resp *http.Response, policy ProviderResponsePolicy) error {
	...
	if _, err := ReadProviderJSONDocument(t.Context(), resp, policy, contract.ProviderHolodex); err != nil {
		return fmt.Errorf("read provider JSON document: %w", err)
	}
