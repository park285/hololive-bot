...
func TestRunnerRejectsSchemaDrift(t *testing.T) {
	t.Parallel()
	_, err := NewRunner(&staticFetcher{body: []byte(`{"foo":[]}`)}).Collect(context.Background(), officialInput(t))
...
func TestRunnerRejectsNonEmptyPayloadWhenAllRowsInvalid(t *testing.T) {
	...
	output, err := NewRunner(&staticFetcher{body: body}).Collect(context.Background(), officialInput(t))
...
func TestRunnerDoesNotPublishOnTimeout(t *testing.T) {
	t.Parallel()
	output, err := NewRunner(&staticFetcher{err: collecterr.New(collecterr.Timeout, collecterr.ClassTimeout, "timeout")}).Collect(context.Background(), officialInput(t))
...
func officialInput(t testing.TB) *collectutil.RunInput {
	t.Helper()
	...
	targets := testutil.TargetSnapshot(t, &spec, job, map[contract.ObservationKind][]string{
