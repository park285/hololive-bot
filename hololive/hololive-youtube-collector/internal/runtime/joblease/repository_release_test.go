...
func TestReleaseReasonsUseDistinctShapesAndPreserveFailureHistory(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			assertReleasedLeaseShape(t, pool, spec.JobKey, scheduledFor, testCase.state, testCase.errorCode)
			assertFailureDiagnostics(t, pool, spec.JobKey, prior.code, prior.class, prior.detail, prior.at)
...
func TestReleaseReasonRejectsUnknownCode(t *testing.T) {
	...
	lease := &JobLease{}
	if err := lease.Release(t.Context(), ReleaseReason("not_a_release")); err == nil {
