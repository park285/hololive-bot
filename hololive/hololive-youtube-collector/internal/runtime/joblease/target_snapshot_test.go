...
func TestExactTargetSnapshotPreservesExplicitDisabledAndDefensiveCopies(t *testing.T) {
	...
	snapshot, err := newExactTargetSnapshot(7, "UC_A", requested, map[contract.ObservationKind]bool{
	...
	if allowed, err := snapshot.Allows(contract.KindVideoList, "UC_A"); err != nil || !allowed {
		t.Fatalf("enabled exact target = %t, %v", allowed, err)
	}
	if allowed, err := snapshot.Allows(contract.KindShortsList, "UC_A"); err != nil || allowed {
		t.Fatalf("disabled exact target = %t, %v", allowed, err)
	}
	...
	if len(roster) != 1 || roster[0] != "UC_A" {
		t.Fatalf("defensive roster = %#v", roster)
	}
	if _, err := snapshot.Allows(contract.KindSchedule, "UC_A"); err == nil {
...
func TestProjectionTargetSnapshotPreservesEmptyRosterAndCap(t *testing.T) {
	...
	snapshot, err := newProjectionTargetSnapshot(9, requested, map[contract.ObservationKind][]string{
		contract.KindLiveSnapshot: {"UC_B", "UC_A"},
	...
	if len(roster) != 2 || roster[0] != "UC_A" || roster[1] != "UC_B" {
	...
	_, err = newProjectionTargetSnapshot(9, requested, map[contract.ObservationKind][]string{
		contract.KindLiveSnapshot: {"UC_A", "UC_B"},
...
func TestTargetSnapshotRejectsDuplicatesMissingKindsAndInvalidGeneration(t *testing.T) {
	_, err := newProjectionTargetSnapshot(1, []contract.ObservationKind{contract.KindLiveSnapshot}, map[contract.ObservationKind][]string{
		contract.KindLiveSnapshot: {"UC_A", "UC_A"},
	}, 2)
	...
	_, err = newExactTargetSnapshot(0, "UC_A", []contract.ObservationKind{contract.KindLiveSnapshot}, nil)
	if collecterr.ClassOf(err) != collecterr.ClassInternal {
		t.Fatalf("generation error = %v", err)
	}
	_, err = newExactTargetSnapshot(1, "UC_A", []contract.ObservationKind{
...
func TestTargetSnapshotValidateRequestedAndClone(t *testing.T) {
	snapshot, err := newProjectionTargetSnapshot(2, []contract.ObservationKind{contract.KindLiveSnapshot}, map[contract.ObservationKind][]string{
		contract.KindLiveSnapshot: {"UC_A"},
	}, 1)
	if err != nil {
		t.Fatal(err)
	}
	if err := snapshot.ValidateRequested([]contract.ObservationKind{contract.KindLiveSnapshot}); err != nil {
		t.Fatal(err)
	}
	if err := snapshot.ValidateRequested([]contract.ObservationKind{contract.KindSchedule}); err == nil {
	...
	allowed, err := snapshot.Allows(contract.KindLiveSnapshot, "UC_A")
	if err != nil || !allowed {
...
func TestBuildJobKeyAndExpectedLeaseSubjectPreservePersistedGlobalIdentity(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{sourceobservation.JobID{Provider: contract.ProviderHololiveOfficial, Kind: "official_schedule"}, "global:hololive-schedule", "collector:hololive_official:official_schedule:global"},
	} {
	...
	subject, err := ExpectedLeaseSubject(subjectJob, "UC_A")
	if err != nil || subject != "UC_A" {
