...
var readinessHTTPStatusTable = map[ReadinessState]struct {
	...
	ReadyStarting:          {status: http.StatusServiceUnavailable, ready: "not_ready", dependency: true},
	ReadyWaitingCollection: {status: http.StatusServiceUnavailable, ready: "not_ready", dependency: true},
	ReadyWaitingHandoff:    {status: http.StatusServiceUnavailable, ready: "not_ready", dependency: true},
	ReadyDegraded:          {status: http.StatusServiceUnavailable, ready: "not_ready", dependency: true},
...
func TestRDY001NilDependenciesAreDegradedWithExactName(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if body.Status != "not_ready" || body.State != ReadyDegraded || body.Dependency != test.dependency {
...
func TestRDY002HelperHealthTimeoutFailsInsideTotalBudget(t *testing.T) {
	...
	if body.Status != "not_ready" || body.Dependency != "youtubejs" || readinessHTTPStatus(&body) != http.StatusServiceUnavailable {
...
func TestRDY003PendingCountUsesCapSentinel(t *testing.T) {
	...
	if _, err := (&postgresQueueStore{}).CountPending(context.Background(), pendingQueueCap); err == nil {
...
func TestRDY010UnknownOrMismatchedHandoffShapeIsDegraded(t *testing.T) {
	...
	body := evaluateReadiness(context.Background(), &deps)
	if body.State != ReadyDegraded || body.Dependency != "postgres_queue" || body.PendingQueue != nil || body.Status != "not_ready" {
