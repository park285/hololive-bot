...
func TestSCH003QueueFullRollsBackMarkAndIncrementsMetric(t *testing.T) {
	...
	if result := scheduler.enqueueDiscovered(context.Background(), &spec); result != EnqueueFull {
...
func TestSCH006OneCycleProjectionQueryIsOne(t *testing.T) {
	...
	scheduler.discoverOnce(context.Background())
...
func TestSCH012StopDrainsQueueAndQueuedSet(t *testing.T) {
	...
	for i := range 3 {
		...
		_ = scheduler.enqueue(context.Background(), &spec)
	}
	if err := scheduler.Stop(context.Background()); err != nil {
		t.Fatal(err)
	}
...
func TestSCH013SnapshotIsConsistentUnderOneLock(t *testing.T) {
	...
	snap := scheduler.Snapshot()
	snap.QueueDepth = 99
	if scheduler.Snapshot().QueueDepth == 99 {
