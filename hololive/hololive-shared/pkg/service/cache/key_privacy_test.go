...
// 이 경로의 회귀는 Valkey 장애 중에만 드러난다. miniredis를 세운 뒤 끊어야 Service가 실제 실패 로그를
// 찍는 상태에 들어간다.
func outageService(t *testing.T) (*Service, *bytes.Buffer) {
	...
	service, err := NewCacheService(context.Background(), Config{
		Host:              host,
		Port:              port,
		DisableCache:      true,
...
func TestCacheOutageLogsNeverCarryRoomPlaintext(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), privacyOutageWait)
	defer cancel()
