...
func TestLiveConsumerUpcomingLiveEndedPersistsOnce(t *testing.T) {
	...
	if status := liveSessionStatus(t, pool, "vid-a"); status != string(domain.LiveStatusEnded) {
...
func TestLiveConsumerFutureSkewedSourceEventUsesScheduledSlot(t *testing.T) {
	...
	if err != nil {
		...
		_ = pool.QueryRow(ctx, `SELECT status, COALESCE(last_error_detail, '') FROM source_observation_queue LIMIT 1`).Scan(&status, &detail)
...
func TestLiveConsumerTwoScopedAbsenceSlotsAfterGraceEnd(t *testing.T) {
	...
	if liveSessionStatus(t, pool, "vid-a") != string(domain.LiveStatusEnded) {
...
func TestLiveConsumerAlreadyEndedLateLiveStaysEnded(t *testing.T) {
	...
	seen := liveLastSeen(t, pool, "vid-a")
...
func liveSessionStatus(t *testing.T, pool *pgxpool.Pool, videoID string) string {
	t.Helper()
	var status string
	if err := pool.QueryRow(context.Background(), `SELECT status FROM youtube_live_sessions WHERE video_id = $1`, videoID).Scan(&status); err != nil {
		t.Fatalf("load live status: %v", err)
	}
	return status
}

func liveLastSeen(t *testing.T, pool *pgxpool.Pool, videoID string) time.Time {
	t.Helper()
	var seen time.Time
	if err := pool.QueryRow(context.Background(), `SELECT last_seen_at FROM youtube_live_sessions WHERE video_id = $1`, videoID).Scan(&seen); err != nil {
