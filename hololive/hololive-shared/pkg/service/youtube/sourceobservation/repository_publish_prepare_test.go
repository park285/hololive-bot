...
func TestClonePublishBatchInputCopiesLeaseObservationsAndCursors(t *testing.T) {
	...
	sourceEvent := time.Date(2026, time.August, 14, 1, 0, 1, 0, time.UTC)
	...
	input.Lease.JobKey = testMutatedValue

	input.Observations[0].Payload[0] ^= 0xff

	input.Observations[0].SubjectKey = testMutatedValue

	input.Checkpoint.Entries[0].Cursor[0] ^= 0xff

	if cloned.Lease.JobKey != "job" || cloned.Observations[0].SubjectKey != testChannelID {
...
type preflightBoundsCase struct {
	name       string
	input      *PublishBatchInput
	wantDetail string
}

func preflightBoundsCases() []preflightBoundsCase {
	countDetail := fmt.Sprintf("checkpoint count must equal observation count and be at most %d", MaxCheckpointCount)
	latencyDetail := "collection latency is outside the accepted range"

	return []preflightBoundsCase{
		{
...
func requirePreflightBounds(t *testing.T, test preflightBoundsCase) {
	t.Helper()
...
func TestPreflightPublishBatchEnforcesBoundsBeforeClone(t *testing.T) {
	...
	for _, test := range preflightBoundsCases() {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()

			requirePreflightBounds(t, test)
		})
	}
}
