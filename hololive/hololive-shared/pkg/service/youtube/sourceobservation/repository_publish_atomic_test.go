...
func TestPUB013InvalidTupleAndTerminalFaultRollBack(t *testing.T) {
	...
	if got := readLeaseTerminal(ctx, t, pool, proof.JobKey); got.state != testSlotStateActive || got.failureCode != "" {
...
func TestPUB014AtomicDeferStoresConstructorDetailUnchanged(t *testing.T) {
	...
	atomic := readLeaseTerminal(ctx, t, pool, proof.JobKey)
...
func readLeaseTerminal(ctx context.Context, t *testing.T, pool *pgxpool.Pool, jobKey string) leaseTerminalState {
	t.Helper()

	var (
		got                                                 leaseTerminalState
		errorCode, failureCode, failureClass, failureDetail sql.NullString
...
func seedPriorLeaseFailure(ctx context.Context, t *testing.T, pool *pgxpool.Pool, jobKey string) leaseTerminalState {
	t.Helper()
