...
func loadStatsState(ctx context.Context, tx dbx.Tx, channelID string, scheduledFor time.Time) (stats.State, error) {
	...
	if err := scanStatsEvidence(rows, &state); err != nil {
		return stats.State{}, fmt.Errorf("%w", err)
	}

	return state, nil
}

func scanStatsEvidence(rows pgx.Rows, state *stats.State) error {
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			return fmt.Errorf("scan channel stats evidence: %w", err)
		}
	...
	if err := rows.Err(); err != nil {
		return fmt.Errorf("load channel stats evidence: %w", err)
	}
...
func persistStatsDecision(ctx context.Context, tx dbx.Tx, observation *Observation, decision *stats.Decision) error {
	if err := persistStatsEvidence(ctx, tx, observation, decision); err != nil {
		return fmt.Errorf("persist stats evidence: %w", err)
	}

	if err := persistStatsHead(ctx, tx, decision); err != nil {
		return fmt.Errorf("persist stats head: %w", err)
	}

	if err := persistStatsSnapshots(ctx, tx, decision); err != nil {
		return fmt.Errorf("persist stats snapshots: %w", err)
	}

	if err := persistStatsConflict(ctx, tx, observation, decision); err != nil {
		return fmt.Errorf("persist stats conflict: %w", err)
	}
