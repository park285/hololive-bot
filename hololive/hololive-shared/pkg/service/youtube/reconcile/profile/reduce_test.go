...
func TestReduceAbsentFieldDoesNotClear(t *testing.T) {
	t.Parallel()
	first := profileAt(1, t1(), Field{Present: true, Value: "hello"}, Field{}, Field{Present: true, Value: "2019-01-02"})
	second := profileAt(2, t2(), Field{}, Field{}, Field{})
	got := mustReduceAll(t, State{}, []Evidence{first, second}, Policy{})
...
func TestReduceExplicitEmptyRequiresStability(t *testing.T) {
	t.Parallel()
	seed := profileAt(1, t1(), Field{Present: true, Value: "hello"}, Field{}, Field{})
	empty1 := profileAt(2, t2(), Field{Present: true, Value: ""}, Field{}, Field{})
	disabled := mustReduceAll(t, State{}, []Evidence{seed, empty1}, Policy{})
	...
	enabled := mustReduceAll(t, State{}, []Evidence{seed, empty1}, Policy{ClearMinObservations: 2, ClearStability: time.Hour})
	if enabled.Head.Description.Value != "hello" {
		t.Fatalf("single empty must not clear: %#v", enabled.Head.Description)
	}
	empty2 := profileAt(3, t2().Add(2*time.Hour), Field{Present: true, Value: ""}, Field{}, Field{})
	cleared := mustReduceAll(t, State{}, []Evidence{seed, empty1, empty2}, Policy{ClearMinObservations: 2, ClearStability: time.Hour})
...
func TestReduceJoinedDateConflictIsRejectedAndRecorded(t *testing.T) {
	t.Parallel()
	first := profileAt(1, t1(), Field{}, Field{}, Field{Present: true, Value: "2019-01-02"})
	second := profileAt(2, t2(), Field{}, Field{}, Field{Present: true, Value: "2020-03-04"})
	got := mustReduceAll(t, State{}, []Evidence{first, second}, Policy{})
...
func TestReduceProfilePermutationsYieldSameProjection(t *testing.T) {
	...
	forward := mustReduceAll(t, State{}, []Evidence{a, b}, Policy{})
	reverse := mustReduceAll(t, State{}, []Evidence{b, a}, Policy{})
...
func profileAt(id int64, at time.Time, description, country, joined Field) Evidence {
	return Evidence{
		ObservationID: id,
		Provider:      contract.ProviderYouTubeJS,
		Sample: Sample{
			ChannelID: "UC_TEST", Description: description, Country: country, JoinedDate: joined,
