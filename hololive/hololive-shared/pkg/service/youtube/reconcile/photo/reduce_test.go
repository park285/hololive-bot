...
const (
	testKindAvatar = "avatar"
	testPhotoURL   = "https://img.test/a.jpg"
	testMediaID    = "media-1"
)
...
func TestReduceCopiesInputBackingStorage(t *testing.T) {
	...
	state := State{ChannelID: "UC_TEST", Head: Head{ChannelID: "UC_TEST", Kinds: map[string]Canonical{
		testKindAvatar: {Identity: "id:media-1", URL: testPhotoURL, EffectiveAt: &effective},
	}}}
	evidence := photoAt(1, t2(), Variant{Kind: testKindAvatar, URL: testPhotoURL, StableMediaID: testMediaID})
	...
	if decision.Sample.Variants[0].URL != testPhotoURL {
		t.Fatal("decision shares evidence variants")
	}

	canonical := decision.Head.Kinds[testKindAvatar]

	*canonical.EffectiveAt = effective.Add(time.Hour)
	decision.Head.Kinds[testKindAvatar] = canonical

	if !state.Head.Kinds[testKindAvatar].EffectiveAt.Equal(originalEffective) {
...
func TestReduceSameIdentityNewURLCreatesNoChangeEvent(t *testing.T) {
	...
	first := photoAt(1, t1(), Variant{
		Kind: testKindAvatar, URL: "https://img.test/a.jpg?s=88", Width: 88, Height: 88, StableMediaID: testMediaID,
	})
	second := photoAt(2, t2(), Variant{
		Kind: testKindAvatar, URL: "https://img.test/a.jpg?s=800", Width: 800, Height: 800, StableMediaID: testMediaID,
	})
...
func TestReducePhotoWithoutStableIDOrFingerprintCannotChangeCanonical(t *testing.T) {
	...
	got := mustReduceAll(t, []Evidence{photoAt(1, t1(), Variant{
		Kind: testKindAvatar, URL: "https://img.test/a.jpg?s=88", Width: 88, Height: 88,
	})}, Policy{ChangeMinObservations: 2, ChangeStability: time.Hour})
	if got.Head.Kinds[testKindAvatar].Identity != "" || len(got.WriteProduct) != 0 {
...
func TestReduceDifferentPhotoIdentityRequiresStabilityThreshold(t *testing.T) {
	...
	first := photoAt(1, t1(), Variant{
		Kind: testKindAvatar, URL: testPhotoURL, Width: 88, Height: 88, StableMediaID: testMediaID,
	})
	next := photoAt(2, t2(), Variant{
		Kind: testKindAvatar, URL: "https://img.test/b.jpg", Width: 88, Height: 88, StableMediaID: "media-2",
	})
	disabled := mustReduceAll(t, []Evidence{first, firstAtLater(), next}, Policy{})

	if disabled.Head.Kinds[testKindAvatar].Identity != "" {
		t.Fatalf("disabled change must keep empty canonical: %#v", disabled.Head.Kinds[testKindAvatar])
	}

	pending := mustReduceAll(t, []Evidence{first, firstAtLater(), next}, Policy{ChangeMinObservations: 2, ChangeStability: time.Hour})
	if pending.Head.Kinds[testKindAvatar].Identity != "id:media-1" {
		t.Fatalf("first stable identity = %#v", pending.Head.Kinds[testKindAvatar])
	}
	...
	if changed.Head.Kinds[testKindAvatar].Identity != "id:media-2" {
		t.Fatalf("stable identity change = %#v", changed.Head.Kinds[testKindAvatar])
	}
}
...
func TestReduceDoesNotSynthesizeFingerprintFromURL(t *testing.T) {
	...
	variant := Variant{Kind: testKindAvatar, URL: "https://img.test/a.jpg?s=88", Width: 88, Height: 88}
	if Identity(&variant) != "" {
...
func TestReducePhotoPermutationsYieldSameProjection(t *testing.T) {
	...
	a := photoAt(1, t1(), Variant{
		Kind: testKindAvatar, URL: "https://img.test/a.jpg?keep=1", Width: 88, Height: 88, StableMediaID: testMediaID,
	})
	...
	if forward.Head.Kinds[testKindAvatar].Identity != reverse.Head.Kinds[testKindAvatar].Identity ||
	...
	if !strings.Contains(forward.Head.Kinds[testKindAvatar].URL, "keep=1") {
		t.Fatalf("reconciler stripped query params: %s", forward.Head.Kinds[testKindAvatar].URL)
	}
}

func t1() time.Time { return time.Date(2026, time.August, 14, 1, 0, 0, 0, time.UTC) }
func t2() time.Time { return time.Date(2026, time.August, 14, 7, 0, 0, 0, time.UTC) }
