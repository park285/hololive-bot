...
func insertPollerTestLiveSession(ctx context.Context, db *pgxpool.Pool, row *domain.YouTubeLiveSession) (int64, error) {
	...
	return execPollerTestInsert(ctx, db, `
		INSERT INTO youtube_live_sessions
			(video_id, channel_id, status, title, scheduled_start_time, started_at, ended_at, live_first_seen_at, is_premiere, topic_id, thumbnail_url, last_seen_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		row.VideoID, row.ChannelID, row.Status, row.Title, row.ScheduledStartTime, row.StartedAt, row.EndedAt, row.LiveFirstSeenAt, row.IsPremiere, row.TopicID, row.ThumbnailURL, row.LastSeenAt)
}
...
func pollerTestSelectColumns(table string) string {
	...
	case "youtube_live_sessions":
		return "video_id, channel_id, status, title, scheduled_start_time, started_at, ended_at, live_first_seen_at, is_premiere, topic_id, thumbnail_url, last_seen_at"
	default:
		return "*"
	}
}
