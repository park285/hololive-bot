...
import (
	...
	"github.com/kapu/hololive-shared/pkg/service/youtube/admission"
...
func (s *Scheduler) deferJobClaim(ctx context.Context, job *Job, claim polling.JobClaim, pollErr error) error {
	retryAfter := admissionRetryAfterFromError(pollErr, s.errorBackoffMin)
	if retryAfter <= 0 {
		retryAfter = s.errorBackoffMin
	}
	...
	result := polling.BoolResult(deferred, err)
