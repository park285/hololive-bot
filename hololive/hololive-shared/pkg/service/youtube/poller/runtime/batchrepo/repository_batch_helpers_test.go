...
func (db *batchTestDB) resetOptionalTables(t *testing.T, models ...any) {
	...
	for _, table := range []string{
		...
		_, err := db.Pool.Exec(t.Context(), "DROP TABLE IF EXISTS "+table+" CASCADE")
		require.NoError(t, err)
	}
}

func persistVideos(ctx context.Context, repository BatchRepository, videos []*domain.YouTubeVideo, notifications []*domain.YouTubeNotificationOutbox, watermark *domain.YouTubeContentWatermark) error {
	if err := repository.PersistVideos(ctx, videos, notifications, nil, watermark); err != nil {
		return fmt.Errorf("persist videos: %w", err)
	}

	return nil
}

func persistCommunityPosts(ctx context.Context, repository BatchRepository, posts []*domain.YouTubeCommunityPost, notifications []*domain.YouTubeNotificationOutbox, watermark *domain.YouTubeContentWatermark) error {
	if err := repository.PersistCommunityPosts(ctx, posts, notifications, nil, watermark); err != nil {
		return fmt.Errorf("persist community posts: %w", err)
	}
...
func insertBatchTestValue(ctx context.Context, db *pgxpool.Pool, value any) (int64, error) {
	...
	if isReflectSlice(v) {
		out, err := insertBatchTestSlice(ctx, db, v)
		if err != nil {
			return out, fmt.Errorf("insert batch test slice: %w", err)
		}
	...
	case *domain.YouTubeNotificationOutbox:
		out, err := insertOutbox(ctx, db, row)
		if err != nil {
			return out, fmt.Errorf("insert outbox: %w", err)
		}

		return out, nil
	case *domain.YouTubeNotificationDelivery:
		out, err := insertNotificationDelivery(ctx, db, row)
		if err != nil {
			return out, fmt.Errorf("insert notification delivery: %w", err)
		}

		return out, nil
	case *domain.YouTubeContentAlarmTracking:
		out, err := insertContentAlarmTracking(ctx, db, row)
		if err != nil {
			return out, fmt.Errorf("insert content alarm tracking: %w", err)
		}

		return out, nil
	case *domain.YouTubeCommunityShortsAlarmState:
		out, err := insertCommunityShortsAlarmState(ctx, db, row)
		if err != nil {
			return out, fmt.Errorf("insert community shorts alarm state: %w", err)
		}
...
func insertNotificationDelivery(ctx context.Context, db *pgxpool.Pool, row *domain.YouTubeNotificationDelivery) (int64, error) {
	out, err := execInsert(ctx, db, `
	...
	if err != nil {
		return out, fmt.Errorf("exec insert: %w", err)
	}

	return out, nil
}

func insertContentAlarmTracking(ctx context.Context, db *pgxpool.Pool, row *domain.YouTubeContentAlarmTracking) (int64, error) {
	if row.DeliveryStatus == "" {
		row.DeliveryStatus = domain.YouTubeContentAlarmDeliveryStatusPending
	}

	applyBatchTestTimestamps(&row.CreatedAt, &row.UpdatedAt)
...
func insertCommunityShortsAlarmState(ctx context.Context, db *pgxpool.Pool, row *domain.YouTubeCommunityShortsAlarmState) (int64, error) {
	if row.DeliveryStatus == "" {
...
func applyBatchTestTimestamps(createdAt, updatedAt *time.Time) {
	now := time.Now()
...
func insertBatchTestSlice(ctx context.Context, db *pgxpool.Pool, values reflect.Value) (int64, error) {
	...
	for i := range values.Len() {
		...
		if err != nil {
			return rows, fmt.Errorf("insert batch test value: %w", err)
		}
