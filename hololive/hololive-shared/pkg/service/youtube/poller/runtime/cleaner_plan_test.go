...
func TestViewerSampleCleanerDeleteBatchSkipsLockedSession(t *testing.T) {
	...
	insertViewerSampleCleanerLiveSession(t, pool, "a-locked", domain.LiveStatusEnded, now.AddDate(0, 0, -9))
	insertViewerSampleCleanerLiveSession(t, pool, "b-available", domain.LiveStatusEnded, now.AddDate(0, 0, -8))
	insertViewerSampleCleanerSample(t, pool, "a-locked", now)
	insertViewerSampleCleanerSample(t, pool, "b-available", now)
	...
	require.EqualValues(t, 1, countViewerSampleCleanerSamples(t, pool, "a-locked"))
	require.EqualValues(t, 0, countViewerSampleCleanerSamples(t, pool, "b-available"))
}
...
func TestViewerSampleCleanerPlanBoundsIneligiblePrefixAndPaginatesEmptySessions(t *testing.T) {
	...
	insertViewerSampleCleanerLiveSession(t, pool, ineligibleVideoID, domain.LiveStatusLive, oldEndedAt)
	insertViewerSampleCleanerLiveSession(t, pool, eligibleVideoID, domain.LiveStatusEnded, oldEndedAt)
	insertViewerSampleCleanerSessions(t, pool, "empty-", emptyEndedSessions, domain.LiveStatusEnded, oldEndedAt)
	insertViewerSampleCleanerSessions(t, pool, "live-", liveSessions, domain.LiveStatusLive, oldEndedAt)
	insertViewerSampleCleanerSamples(t, pool, ineligibleVideoID, now, ineligibleSamples)

	for i := range 3 {
		insertViewerSampleCleanerSample(t, pool, eligibleVideoID, now.Add(time.Duration(i)*time.Second))
	}

	_, err := pool.Exec(ctx, "ANALYZE youtube_live_sessions, youtube_live_viewer_samples")
	require.NoError(t, err)

	rawPlan := captureViewerSampleCleanupPlan(t, pool)
	...
	require.EqualValues(t, ineligibleSamples, countViewerSampleCleanerSamples(t, pool, ineligibleVideoID))
	require.EqualValues(t, 0, countViewerSampleCleanerSamples(t, pool, eligibleVideoID))
}

func captureViewerSampleCleanupPlan(t *testing.T, pool *pgxpool.Pool) string {
	t.Helper()
