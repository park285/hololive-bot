...
func (c *Client) getRecentVideosFromRSS(ctx context.Context, channelID string, maxResults int, policy ...FetchPolicy) ([]*parser.Video, error) {
	if maxResults <= 0 {
		return []*parser.Video{}, nil
	}
...
func (c *Client) GetVideoMetadata(ctx context.Context, channelID, videoID string) (parser.VideoMetadata, error) {
	html, err := c.getVideoWatchHTML(ctx, channelID, videoID)
	if err != nil {
		return parser.VideoMetadata{}, err
	}

	metadata := parser.ExtractVideoMetadataFromHTML(html)
	if metadata.Replay == parser.ReplayStatusUnknown {
		return metadata, fmt.Errorf("extract video replay status %s: %w", videoID, parser.ErrReplayStatusNotFound)
	}
