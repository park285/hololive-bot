...
func TestRateLimiterTryReserveWithBucket_LocalDeniedDoesNotBlock(t *testing.T) {
	...
	if decision, err := limiter.TryReserveWithBucket(t.Context(), "bucket"); err != nil || !decision.Allowed {
		t.Fatalf("first reserve = (%+v, %v), want allowed", decision, err)
	}

	start := time.Now()
	decision, err := limiter.TryReserveWithBucket(t.Context(), "bucket")
	...
	if decision.Allowed {
		t.Fatal("second reserve allowed unexpectedly")
	}
...
func TestRateLimiterTryReserveWithBucket_DistributedDeniedRollsBackLocalReservation(t *testing.T) {
	...
	if decision.Allowed {
		t.Fatal("first reserve allowed unexpectedly")
	}
