...
func dropCanonicalUniqueIndex(t *testing.T, db trackingDB) {
	...
	_, err := db.Exec(t.Context(), `DROP INDEX IF EXISTS idx_ycat_kind_canonical_content`)
	require.NoError(t, err)
}
...
func TestMigration070RepointsPrimaryKeyToCanonical(t *testing.T) {
	...
	detectedAt := time.Date(2026, time.April, 10, 1, 0, 0, 0, time.UTC)
...
func TestMigration070DedupesLegacyDuplicateCanonicalRows(t *testing.T) {
	...
	detectedRaw := time.Date(2026, time.April, 10, 1, 0, 0, 0, time.UTC)
	detectedCanonical := time.Date(2026, time.April, 10, 1, 2, 0, 0, time.UTC)
	alarmSentAt := time.Date(2026, time.April, 10, 1, 3, 0, 0, time.UTC)
