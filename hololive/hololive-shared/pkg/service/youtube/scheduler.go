...
func (ys *Scheduler) processMilestones(ctx context.Context, channelID string, member *domain.Member, milestones []uint64, now time.Time) (achieved int, checkErrors int, saveErrors int) {
	achieved = 0
...
func (ys *Scheduler) processChannelStats(ctx context.Context, channelID string, member *domain.Member, currentStats *ChannelStats, now time.Time) channelStatsResult {
...
func (ys *Scheduler) SendMilestoneAlerts(ctx context.Context, sendMessage func(room, message string) error, rooms []string) error {
	...
	for _, m := range milestones {
		msg, err := ys.formatter.FormatMilestoneAchieved(ctx, m.MemberName, util.FormatKoreanNumber(int64(m.Value)))
		if err != nil {
...
func (ys *Scheduler) formatChangeMessageWithContext(ctx context.Context, change *domain.StatsChange) string {
	if change.PreviousStats == nil || change.CurrentStats == nil {
		return ""
	}
...
func (ys *Scheduler) getNearMilestoneChannelMap(
	ctx context.Context,
...
func (ys *Scheduler) formatApproachingMessage(ctx context.Context, memberName string, milestone, currentSubs uint64) string {
