...
func buildPostDeliveryPathUsageRows(scanned []postDeliveryPathUsageScanRow) []analytics.PostDeliveryPathUsage {
	rows := make([]analytics.PostDeliveryPathUsage, 0, len(scanned))
	for i := range scanned {
		rows = append(rows, analytics.PostDeliveryPathUsage{
