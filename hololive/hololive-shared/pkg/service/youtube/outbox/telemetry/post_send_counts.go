...
func buildPostSendCountsFromScanRows(scanned []postSendCountScanRow) []analytics.PostSendCount {
	rows := make([]analytics.PostSendCount, 0, len(scanned))
	for i := range scanned {
		rows = append(rows, analytics.PostSendCount{
