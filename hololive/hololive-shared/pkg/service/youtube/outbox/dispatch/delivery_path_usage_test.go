...
func TestDeliveryTelemetryRepository_ListPostDeliveryPathUsageSince_GroupsByContentAndPath(t *testing.T) {
	...
	byKey := make(map[key]analytics.PostDeliveryPathUsage, len(rows))
	for i := range rows {
		byKey[key{contentID: rows[i].ContentID, deliveryPath: rows[i].DeliveryPath}] = rows[i]
	}

	communityNew, ok := byKey[key{contentID: communityOutbox.ContentID, deliveryPath: telemetry.CommunityShortsDeliveryPath}]
	require.True(t, ok)
	...
	shortRow, ok := byKey[key{contentID: shortOutbox.ContentID, deliveryPath: telemetry.CommunityShortsDeliveryPath}]
	require.True(t, ok)
