...
func TestFetchAndLockClaimsEarliestNextAttemptBeforeEarliestCreated(t *testing.T) {
	...
	outboxID := insertClaimOrderOutbox(ctx, t, pool, now)

	createdFirstID := insertClaimOrderDelivery(ctx, t, pool, outboxID, "room-created-first", now.Add(-20*time.Minute), now.Add(-1*time.Minute))
	dueFirstID := insertClaimOrderDelivery(ctx, t, pool, outboxID, "room-due-first", now.Add(-5*time.Minute), now.Add(-10*time.Minute))
...
func insertClaimOrderOutbox(ctx context.Context, t *testing.T, db *pgxpool.Pool, now time.Time) int64 {
	t.Helper()
...
func insertClaimOrderDelivery(ctx context.Context, t *testing.T, db *pgxpool.Pool, outboxID int64, roomID string, createdAt, nextAttemptAt time.Time) int64 {
	t.Helper()
