...
func TestMarkSentBatchDoesNotBypassActiveLease(t *testing.T) {
	...
	if owner := lockedByOf(ctx, t, repository, items[0].ID); owner == nil || *owner != testWorkerA {
...
func TestMarkSendingChecksLeaseAtDatabaseExecutionTime(t *testing.T) {
	...
	if scanErr := repository.pool.QueryRow(ctx, "SELECT lock_expires_at FROM notification_delivery_outbox WHERE id = $1", items[0].ID).Scan(&lockExpiresAt); scanErr != nil {
		t.Fatalf("load lock expiry: %v", scanErr)
	}
	...
	waitForOutboxQueryLock(ctx, t, repository, tx)
	waitForOutboxDatabaseTime(ctx, t, repository, lockExpiresAt)
...
func TestQuarantineStaleSendingUsesDatabaseExecutionTime(t *testing.T) {
	...
	waitForOutboxQueryLock(ctx, t, repository, tx)
	waitForOutboxDatabaseTime(ctx, t, repository, staleAt)
	...
	if quarantined := countByStatus(ctx, t, repository, deliveryStatusQuarantined); quarantined != 1 {
		t.Fatalf("quarantined status count = %d, want 1", quarantined)
	}
}

func waitForOutboxQueryLock(ctx context.Context, t *testing.T, repository *OutboxRepository, tx pgx.Tx) {
	t.Helper()
...
func waitForOutboxDatabaseTime(ctx context.Context, t *testing.T, repository *OutboxRepository, threshold time.Time) {
	t.Helper()
	require.Eventually(t, func() bool {
		var reached bool
