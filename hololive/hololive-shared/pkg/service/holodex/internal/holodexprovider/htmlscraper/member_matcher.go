...
func (s *Service) parseStreamElement(sel *goquery.Selection, currentDate string) (*domain.Stream, error) {
...
func (s *Service) extractMemberName(sel *goquery.Selection) string {
...
func (s *Service) matchMemberToChannel(memberName string) string {
	if memberName == "" {
		return ""
	}
...
func (s *Service) extractVideoID(videoURL string) string {
...
func (s *Service) parseDatetimeWithContext(date, timeStr string) (*time.Time, error) {
...
func (s *Service) extractMemberFromOnClick(onclick string) string {
