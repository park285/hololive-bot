...
func TestRenameHashMappingMissingTempPreservesExistingHash(t *testing.T) {
	...
	if err := cacheClient.HSet(ctx, sharedalarmkeys.TwitchLoginMapKey, "legacy-login", "UC_legacy"); err != nil {
		t.Fatalf("seed target: %v", err)
	}

	err := mapper.renameHashMappingKey(ctx, "missing-temp", sharedalarmkeys.TwitchLoginMapKey, map[string]any{
		"new-login": "UC_new",
	})
	if err == nil {
	...
	got, err := cacheClient.HGetAll(ctx, sharedalarmkeys.TwitchLoginMapKey)
	if err != nil {
