...
func (m *Mapper) SyncAll(ctx context.Context) error {
	...
	channelIDs, err := m.cache.SMembers(ctx, sharedalarmkeys.AlarmChannelRegistryKey)
	if err != nil {
	...
	if err := m.replaceHashMappingsWithEmptyMarker(ctx, sharedalarmkeys.ChzzkChannelMapKey, sharedalarmkeys.ChzzkChannelMapEmptyKey, chzzkMappings); err != nil {
		return fmt.Errorf("sync chzzk channel mappings: %w", err)
	}

	if err := m.replaceHashMappingsWithEmptyMarker(ctx, sharedalarmkeys.TwitchLoginMapKey, sharedalarmkeys.TwitchLoginMapEmptyKey, twitchMappings); err != nil {
		return fmt.Errorf("sync twitch login mappings: %w", err)
	}

	if err := m.replaceHashMappingsWithEmptyMarker(ctx, sharedalarmkeys.TwitchChannelLoginMapKey, sharedalarmkeys.TwitchChannelLoginMapEmptyKey, twitchChannelMappings); err != nil {
...
func (m *Mapper) SyncForChannel(ctx context.Context, channelID string) error {
	...
	registered, err := m.cache.SIsMember(ctx, sharedalarmkeys.AlarmChannelRegistryKey, channelID)
	if err != nil {
...
func (m *Mapper) removeStaleTwitchLoginMappingIfOwned(ctx context.Context, login, channelID string) error {
	...
	owner, err := m.cache.HGet(ctx, sharedalarmkeys.TwitchLoginMapKey, login)
	if err != nil {
