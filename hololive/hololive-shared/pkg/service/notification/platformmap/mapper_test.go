...
func assertTwitchHashes(t *testing.T, c cache.Client, wantLogin, wantChannel map[string]string) {
	...
	channelMap, err := c.HGetAll(t.Context(), sharedalarmkeys.TwitchChannelLoginMapKey)
	require.NoError(t, err)
...
func TestSyncForChannel_UnregisteredRemovesOwnedTwitchMapping(t *testing.T) {
	...
	chzzkMap, err := c.HGetAll(t.Context(), sharedalarmkeys.ChzzkChannelMapKey)
	require.NoError(t, err)
...
func TestSyncForChannel_RegisteredUnknownChannelClearsMappings(t *testing.T) {
	...
	require.NoError(t, c.HSet(t.Context(), sharedalarmkeys.TwitchLoginMapKey, "ghostlogin", "UC_ghost"))
	require.NoError(t, c.HSet(t.Context(), sharedalarmkeys.TwitchChannelLoginMapKey, "UC_ghost", "ghostlogin"))
	require.NoError(t, c.HSet(t.Context(), sharedalarmkeys.ChzzkChannelMapKey, "UC_ghost", "chzzk_ghost"))
...
func TestClearConflictingTwitchChannelLoginMapping(t *testing.T) {
	...
	mapper, c := newTestMapper(t)
	require.NoError(t, c.HSet(t.Context(), sharedalarmkeys.TwitchLoginMapKey, "sharedlogin", "UC_prev_owner"))
	require.NoError(t, c.HSet(t.Context(), sharedalarmkeys.TwitchChannelLoginMapKey, "UC_prev_owner", "sharedlogin"))
	require.NoError(t, c.HSet(t.Context(), sharedalarmkeys.TwitchChannelLoginMapKey, "UC_keep", "other"))
