...
func NewSummarizer(llm LLMClient, searcher WebSearcher, validator *SourceValidator, logger *slog.Logger) *SummarizerImpl {
	if logger == nil {
		logger = slog.Default()
	}
...
func (s *SummarizerImpl) Summarize(ctx context.Context, input SummarizeInput) (*Digest, error) {
	if len(input.Candidates) == 0 {
		return &Digest{
			Period:       input.Period,
			Headline:     defaultHeadline(input.Period),
...
func validateAndBuildDigestFromResponse(
	...
	return &Digest{
		Period:       NormalizePeriod(Period(response.Period)),
		Headline:     headline,
...
func BuildDeterministicFallback(period Period, candidates []FilteredCandidate) *Digest {
	...
	return &Digest{
		...
		Headline:     defaultHeadline(period),
		TopItems:     items,
		MoreSummary:  moreSummary,
...
func buildSearchQuery(period Period, roomMembers []string, now time.Time) string {
	periodText := "weekly"
	if NormalizePeriod(period) == PeriodMonthly {
		periodText = "monthly"
	}
