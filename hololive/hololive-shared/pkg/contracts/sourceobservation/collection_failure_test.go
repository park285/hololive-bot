...
func TestAPI008FailureDiagnosticHasNoPublicMutableSurface(t *testing.T) {
	...
	_ = diagnostic.Detail() + testMutatedMarker
...
func TestTupleGettersReturnSortedDefensiveCopies(t *testing.T) {
	...
	tuples[0].Code = testMutatedMarker
	...
	if freshDefer[0].Code == testMutatedMarker {
	...
	complete[0].Class = testMutatedMarker
	...
	if freshComplete[0].Class == testMutatedMarker {
