...
func (proof *LeaseProof) validate(scheduledFor time.Time) error {
	...
	if proof.FenceEpoch <= 0 || proof.ProjectionGeneration <= 0 {
		return errors.New("validate source observation envelope: lease fence and projection generation must be positive")
	}

	if proof.ScheduledFor.IsZero() || !proof.ScheduledFor.Equal(scheduledFor) {
		return errors.New("validate source observation envelope: lease scheduled slot mismatch")
	}
