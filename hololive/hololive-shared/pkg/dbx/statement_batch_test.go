...
const (
	testStatementFirstSQL  = "first"
	testStatementSecondSQL = "second"
)
...
func (t *statementRecordingBatchTx) SendBatch(ctx context.Context, batch *pgx.Batch) pgx.BatchResults {
	...
	result := &statementRecordingResults{
		owner: t,
		batch: batch,
...
func (r *statementRecordingResults) Exec() (pgconn.CommandTag, error) {
	...
	return r.exec(query.SQL, query.Arguments...)
}
...
func TestExecStatementsSequentialFallback(t *testing.T) {
	...
	statements := []Statement{{SQL: testStatementFirstSQL}, {SQL: testStatementSecondSQL}, {SQL: "third"}}

	if err := ExecStatements(t.Context(), tx, statements); !errors.Is(err, failure) {
		t.Fatalf("fallback error=%v", err)
	}

	if !slices.Equal(tx.executed, []string{testStatementFirstSQL, testStatementSecondSQL}) {
...
func TestExecStatementsCloseFailureIsReturned(t *testing.T) {
	...
	if err := ExecStatements(t.Context(), tx, []Statement{{SQL: testStatementFirstSQL}}); !errors.Is(err, failure) {
...
func TestExecStatementsPanicClosesResults(t *testing.T) {
	...
	if err := ExecStatements(t.Context(), tx, []Statement{{SQL: testStatementFirstSQL}}); err != nil {
		t.Fatal(err)
	}
}
...
func TestExecStatementsCanceledContextDoesNotStartWork(t *testing.T) {
	...
	for _, tx := range []Tx{sequential, batched} {
		if err := ExecStatements(ctx, tx, []Statement{{SQL: testStatementFirstSQL}}); !errors.Is(err, context.Canceled) {
...
func TestExecStatementsRejectsNilBatchResults(t *testing.T) {
	if err := ExecStatements(t.Context(), &nilStatementBatchTx{}, []Statement{{SQL: testStatementFirstSQL}}); err == nil {
