...
func (c *YouTubePlaneConfig) Validate() error {
	...
	if err := c.validateConsumers(); err != nil {
		return fmt.Errorf("validate consumers: %w", err)
	}

	if err := c.validateClaimAndTimeouts(); err != nil {
		return fmt.Errorf("validate claim and timeouts: %w", err)
	}

	if err := c.validateProjectionAndFinalizer(); err != nil {
		return fmt.Errorf("validate projection and finalizer: %w", err)
	}

	if err := c.validateRetentionAndReplay(); err != nil {
		return fmt.Errorf("validate retention and replay: %w", err)
	}

	if err := c.validateStabilityPolicies(); err != nil {
		return fmt.Errorf("validate stability policies: %w", err)
	}
...
func (c *YouTubePlaneConfig) validateRetentionAndReplay() error {
	if err := c.validateRetention(); err != nil {
		return fmt.Errorf("validate retention: %w", err)
	}
...
func (c *YouTubePlaneConfig) validateStabilityPolicies() error {
	if err := c.validateContentAbsenceGrace(); err != nil {
		return fmt.Errorf("validate content absence grace: %w", err)
	}
	...
	if err := c.validatePhotoChange(); err != nil {
		return fmt.Errorf("validate photo change: %w", err)
	}
...
func (c *YouTubePlaneConfig) validateClaimAndTimeouts() error {
	...
	if err := c.validateTransactionTimeout(); err != nil {
		return fmt.Errorf("validate transaction timeout: %w", err)
	}
	...
	if err := c.validateShutdownWindows(); err != nil {
		return fmt.Errorf("validate shutdown windows: %w", err)
	}
...
func (c *YouTubePlaneConfig) validateProfileClear() error {
	if err := validateStabilityPair(
		"profile clear",
		c.ProfileClearMinObservations,
		c.ProfileClearStability,
	); err != nil {
		return fmt.Errorf("validate stability pair: %w", err)
	}
