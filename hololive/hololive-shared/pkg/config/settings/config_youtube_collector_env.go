...
func loadYouTubeCollectorConfig() (YouTubeCollectorConfig, error) {
	...
	if cfg.ReadinessTimeout, err = requiredSecondsDurationEnv("YOUTUBE_COLLECTOR_READINESS_TIMEOUT_SECONDS", defaults.ReadinessTimeout); err != nil {
		return YouTubeCollectorConfig{}, fmt.Errorf("required seconds duration env: %w", err)
	}
	...
	if err := loadCollectorYouTubeJSLimits(&cfg, &defaults); err != nil {
		return YouTubeCollectorConfig{}, fmt.Errorf("load collector youtube JS limits: %w", err)
	}

	if err := loadCollectorPaginationLimits(&cfg, &defaults); err != nil {
		return YouTubeCollectorConfig{}, fmt.Errorf("load collector pagination limits: %w", err)
	}
...
func loadCollectorPaginationLimits(cfg, defaults *YouTubeCollectorConfig) error {
	...
	if cfg.RequestInterval, err = requiredSecondsDurationEnv("YOUTUBE_COLLECTOR_REQUEST_INTERVAL_SECONDS", defaults.RequestInterval); err != nil {
		return fmt.Errorf("required seconds duration env: %w", err)
	}
