...
func TestGetChannelStatistics_SecondCallIsServedFromCache(t *testing.T) {
	...
	scraper := &countingScraper{
		...
		stats: map[string]*parser.ChannelStats{
			testChannelID1: {ChannelID: testChannelID1, SubscriberCount: 100, VideoCount: 10, ViewCount: 1000, Handle: "@one"},
			testChannelID2: {ChannelID: testChannelID2, SubscriberCount: 200, VideoCount: 20, ViewCount: 2000, Handle: "@two"},
		},
	}
	cacheClient, writes := newFakeStatsCache()
	ys := newCachedStatsService(scraper, cacheClient)

	first, err := ys.GetChannelStatistics(t.Context(), []string{testChannelID1, testChannelID2})
	if err != nil {
	...
	second, err := ys.GetChannelStatistics(t.Context(), []string{testChannelID1, testChannelID2})
	if err != nil {
	...
	for _, channelID := range []string{testChannelID1, testChannelID2} {
...
func TestGetChannelStatistics_PartialCacheHitOnlyScrapesMissing(t *testing.T) {
	...
	scraper := &countingScraper{
		calls: make(map[string]int),
		stats: map[string]*parser.ChannelStats{
			testChannelID1: {ChannelID: testChannelID1, SubscriberCount: 100},
	...
	if _, err := ys.GetChannelStatistics(t.Context(), []string{testChannelID1}); err != nil {
		t.Fatalf("warm-up GetChannelStatistics() error = %v", err)
	}

	got, err := ys.GetChannelStatistics(t.Context(), []string{testChannelID1, "UC3"})
	if err != nil {
	...
	if calls := scraper.callCount(testChannelID1); calls != 1 {
...
func TestGetChannelStatistics_AllScrapesFailWithoutCacheReturnsError(t *testing.T) {
	...
	scraper := &countingScraper{
		calls: make(map[string]int),
		errs: map[string]error{
			testChannelID1: errScrapeFailed,
			testChannelID2: errScrapeFailed,
		},
	}
	cacheClient, _ := newFakeStatsCache()
	ys := newCachedStatsService(scraper, cacheClient)
...
func TestGetChannelStatistics_CachedEntrySurvivesScrapeFailure(t *testing.T) {
	...
	scraper := &countingScraper{
		calls: make(map[string]int),
		stats: map[string]*parser.ChannelStats{testChannelID1: {ChannelID: testChannelID1, SubscriberCount: 100}},
		errs:  map[string]error{testChannelID2: errScrapeFailed},
	}
	cacheClient, _ := newFakeStatsCache()
	ys := newCachedStatsService(scraper, cacheClient)
	...
	if len(got) != 1 || got[testChannelID1] == nil {
