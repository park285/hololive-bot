...
func TestReduceCopiesInputBackingStorage(t *testing.T) {
	...
	state := State{ChannelID: testChannelID, Head: Head{ChannelID: testChannelID, Description: CanonicalField{
		Set: true, Value: testDescription, EffectiveAt: &effective,
	}}}
	...
	*decision.Head.Description.EffectiveAt = effective.Add(time.Hour)
	...
	if !state.Head.Description.EffectiveAt.Equal(originalEffective) || state.Head.Description.Value != testDescription {
...
func TestReduceAbsentFieldDoesNotClear(t *testing.T) {
	...
	first := profileAt(1, t1(), Field{Present: true, Value: testDescription}, Field{Present: true, Value: "2019-01-02"})
	second := profileAt(2, t2(), Field{}, Field{})
	got := mustReduceAll(t, []Evidence{first, second}, Policy{})

	if !got.Head.Description.Set || got.Head.Description.Value != testDescription {
...
func TestReduceExplicitEmptyRequiresStability(t *testing.T) {
	...
	seed := profileAt(1, t1(), Field{Present: true, Value: testDescription}, Field{})
	empty1 := profileAt(2, t2(), Field{Present: true, Value: ""}, Field{})
	disabled := mustReduceAll(t, []Evidence{seed, empty1}, Policy{})

	if !disabled.Head.Description.Set || disabled.Head.Description.Value != testDescription {
	...
	if enabled.Head.Description.Value != testDescription {
...
func TestReduceProfilePermutationsYieldSameProjection(t *testing.T) {
	...
	a := Evidence{
		...
		Sample: Sample{
			ChannelID: testChannelID, Handle: Field{Present: true, Value: "@one"},
	...
	b := Evidence{
		...
		Sample: Sample{
			ChannelID: testChannelID, Country: Field{Present: true, Value: "jp"},
...
func profileAt(id int64, at time.Time, description, joined Field) Evidence {
	return Evidence{
		...
		Sample: Sample{
			ChannelID: testChannelID, Description: description, JoinedDate: joined,
