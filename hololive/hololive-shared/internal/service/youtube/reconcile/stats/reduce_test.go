...
func TestReduceCopiesInputBackingStorage(t *testing.T) {
	...
	lastAt := t1()
	lastCount := int64(10)
	state := State{ChannelID: testChannelID, Head: Head{
		ChannelID: testChannelID, LastResolvedScheduledFor: &lastAt, LastResolvedSubscriberCount: &lastCount,
	}}
...
func TestReduceHiddenCountRemainsNil(t *testing.T) {
	...
	got := mustReduceAll(t, []Evidence{{
		...
		Sample: Sample{
			ChannelID: testChannelID, SubscriberCovered: true, ViewCovered: true, VideoCovered: true,
...
func TestReduceComplementaryCoverageDoesNotConflict(t *testing.T) {
	...
	partial := Evidence{
		...
		Sample: Sample{
			ChannelID: testChannelID, Provider: contract.ProviderHolodex,
...
func statsAt(id int64, provider contract.Provider, at time.Time, sub, views, videos int64) Evidence {
	return Evidence{
		...
		Provider:      provider,
		Sample: Sample{
			ChannelID: testChannelID, Provider: provider,
