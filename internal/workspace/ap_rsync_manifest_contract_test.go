...
func TestAPRsyncManifestCheckFailsWithoutGoToolchain(t *testing.T) {
	...
	cmd := exec.Command("bash", filepath.Join(root, "scripts/deploy/check-ap-rsync-manifest.sh"), manifest)
	cmd.Dir = root
	cmd.Env = append(os.Environ(), "GO_CMD=hololive-missing-go-command")
