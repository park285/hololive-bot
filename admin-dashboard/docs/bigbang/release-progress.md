...
운영 정본에는 password hash만 있어 자동 로그인에 사용할 평문 인증 입력은 확보하지 못했습니다. 배포 후 사용자가 직접 로그인하는 방식을 안내한 뒤 계속 진행 지시를 받아 그 순서로 진행한다고 알렸습니다. 이번 전환에서는 운영 개방 전 새 로그인·인증된 CSRF·WS 확인을 실제 수행했다고 표시하지 않습니다. 같은 코드의 격리 시험 결과와 운영 health·metadata·익명 거부·image/policy 대조를 구분하고, 배포 후 사용자의 실제 로그인 확인을 남은 인수 항목으로 기록합니다. password hash를 바꾸거나 임의의 운영 인증 세션을 만들어 검사를 대신하지 않습니다.
