...
같은 실행의 Docker 수명 사건을 대조하니 앞선 YouTube 벤치마크의 PostgreSQL과 Ryuk가 Chromium 시작 뒤에 종료·제거됐습니다. 기존 dbtest는 프로세스 종료 뒤 Ryuk가 비동기로 정리하는 계약이며, 벤치마크 스크립트는 그 완료 전에 다음 검사를 시작했습니다. 해당 스크립트에서 지원되는 `TESTCONTAINERS_SESSION_ID`로 이번 호출만의 세션을 지정하고, SDK가 실제 사용한 Docker endpoint에서 그 label의 컨테이너가 사라진 것을 확인한 뒤 완료합니다. 조회는 최대 30초이며 강제 종료 유예는 2초입니다. endpoint 누락·모호함, 조회 실패, 시간 초과는 실패로 남기며 다른 세션이나 컨테이너를 종료하지 않습니다. 외부 disposable DB 경로는 기존 소유권 검증을 유지합니다.

7개 계약 검사는 소유 세션의 정리 완료, 벤치마크 실패, 조회 실패, 시간 초과, endpoint 누락·모호함, 외부 DB의 Docker 미사용을 확인했습니다. 실제 벤치마크→설치·생성→전체 프런트 순서도 정리 완료 뒤 175건·Chromium/Firefox/WebKit 모두 통과했고 skip은 0건입니다. 벤치마크 횟수·예산, 앱 동작, 브라우저 timeout·assertion은 유지합니다. 최종 게시 검사 결과는 별도로 기록합니다.

보조 검토자는 스크립트·자기 검사·게시 단계 등록과 고정 SDK의 label 경로를 읽어 소유권 침범, 실패의 성공 처리 또는 범위 회귀로 확인한 결함이 없다고 보고했습니다. 실제 수정 후 연속 실행은 부모가 검증했습니다.
