...
이후 게시 경로에서는 Chromium 준비 timeout이 다시 발생했습니다. 전체 프런트의 일반·차가운 캐시·동일 설치/생성 순서 진단은 모두 175건과 세 엔진을 통과하여 아직 원인을 확정하지 못했습니다. 실패 시점에만 문서·계약 준비 상태와 최근 15개 자원 경로/응답 상태를 수집하도록 기존 오류 보고를 보완했습니다. 요청 본문·쿠키·자격증명은 수집하지 않으며 대기 상한과 성공 조건은 유지합니다.

후속 실패에서 문서는 visible/complete였으나 `window.contract`가 없고 다수 모듈의 responseStatus가 0임을 확인했습니다. 앱 bootstrap 전에 멈춘 상태이며 비활성 화면 감시가 원인이라는 증거는 없습니다. 정확한 요청 실패 이유와 HTTP 오류 상태를 최대 16개까지 경로와 함께 수집하고, 준비 대기가 끝나면 해당 관측 listener를 해제합니다. 원인을 확인하지 않은 재시도나 timeout 연장은 추가하지 않습니다. 관측 보완 후 Chromium의 전체 세션 시나리오가 통과했고 skip은 0건입니다.
