...
func TestListContainersFiltersToManagedAndSorts(t *testing.T) {
	const body = `[
		...
		{"Id":"2","Names":["/hololive-api"],"Image":"img","Status":"Up","State":"running","Created":2},
	...
	if len(names) != 2 || names[0] != testAdminContainerName || names[1] != testBusinessContainerName {
		t.Fatalf("managed+sorted names = %v, want [admin-dashboard hololive-api]", names)
	}
}
...
func TestListContainersMaps5xxToInternal(t *testing.T) {
	...
	_, err := client.ListContainers(t.Context())
	if err == nil {
	...
	if !ok {
		t.Fatalf("error type = %T, want contract.AppError", err)
	}
...
func TestRestartContainerPinsMethodAndURL(t *testing.T) {
	...
	if gotPath != "/containers/hololive-api/restart" {
		t.Fatalf("action path = %q, want /containers/hololive-api/restart", gotPath)
	}
...
func TestRestartContainerRejectsUnmanagedBeforeRequest(t *testing.T) {
	...
	err := client.RestartContainer(t.Context(), "random-thing")
	if err == nil {
	...
	if !ok || appErr.Status != http.StatusNotFound {
		t.Fatalf("error = %v (%T), want contract.AppError 404", err, err)
	}
...
func TestStopContainerRejectsInfraBeforeRequest(t *testing.T) {
	...
	err := client.StopContainer(t.Context(), "valkey-cache")
	if err == nil {
	...
	if !ok || appErr.Status != http.StatusForbidden {
		t.Fatalf("error = %v (%T), want contract.AppError 403", err, err)
	}
...
func TestRestartInfraAllowed(t *testing.T) {
	...
	if err := client.RestartContainer(t.Context(), "valkey-cache"); err != nil {
...
func TestStopAppContainerAllowed(t *testing.T) {
	...
	if err := client.StopContainer(t.Context(), testBusinessContainerName); err != nil {
		t.Fatalf("StopContainer on app container error = %v, want success", err)
	}

	if gotPath != "/containers/hololive-api/stop" || gotQuery != "t=30" {
		t.Fatalf("action = %q?%q, want /containers/hololive-api/stop?t=30", gotPath, gotQuery)
	}
}

func TestListContainersSetsManagedAndStopBlocked(t *testing.T) {
	const body = `[
		{"Id":"1","Names":["/hololive-api"],"Image":"img","Status":"Up","State":"running","Created":1},
	...
	app, ok := byName[testBusinessContainerName]
	if !ok || !app.Managed || app.StopBlocked {
		t.Fatalf("hololive-api = %+v, want Managed=true StopBlocked=false", app)
	}
...
func TestActionBudgetExceedsListBudget(t *testing.T) {
	...
	if _, err := client.ListContainers(t.Context()); err == nil {
...
func TestActionTimesOutOnHungDocker(t *testing.T) {
	...
	err := client.RestartContainer(t.Context(), testBusinessContainerName)
	if err == nil {
	...
	if !ok || appErr.Status != http.StatusServiceUnavailable {
		t.Fatalf("error = %v (%T), want contract.AppError 503", err, err)
	}
}
...
func TestRestartContainerMaps5xxToInternal(t *testing.T) {
	...
	if !ok || appErr.Status != http.StatusInternalServerError {
		t.Fatalf("error = %v (%T), want contract.AppError 500", err, err)
	}
}
