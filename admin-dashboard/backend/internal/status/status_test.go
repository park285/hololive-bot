...
func TestEndpointClientsSelectH3ForHTTPS(t *testing.T) {
	clients := endpointClients([]ServiceEndpoint{
		{Name: "h3-svc", URL: "https://hololive-admin-api:30006", HealthPath: testHealthPath},
		{Name: "tcp-svc", URL: "http://localhost:30190", HealthPath: testHealthPath},
	}, time.Second)
...
func TestPublishNeverDeadlocksWithRacingConsumer(t *testing.T) {
	...
	go func() {
		//nolint:revive // 채널이 닫힐 때까지 비우기만 하는 소비자라 루프 본문이 비어 있어야 한다.
		for range updates {
		}
