...
func TestListContainersCacheReturnsDeepCopies(t *testing.T) {
	...
	first, err := client.ListContainers(t.Context())
	if err != nil {
	...
	*first[0].Ports[0].PublicPort = 65534
	...
	cached, err := client.ListContainers(t.Context())
	if err != nil {
	...
	*cached[0].Ports[0].PublicPort = 65535
	...
	again, err := client.ListContainers(t.Context())
	if err != nil {
...
func TestDockerActionDrainsErrorBodyForKeepAliveReuse(t *testing.T) {
	...
	for range 2 {
		if err := client.RestartContainer(t.Context(), "hololive-api"); err == nil {
