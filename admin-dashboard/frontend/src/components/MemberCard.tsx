    ...
    <Card className="relative group overflow-hidden border-slate-200 [content-visibility:auto] contain-intrinsic-size-[350px] focus-within:ring-2 focus-within:ring-sky-100 transition-shadow">
      <Card.Header className="pb-3 border-b border-slate-50">
        <div className="flex items-start justify-between">
          ...
          <button
            ...
            className={`rounded-lg p-2 outline-none transition-all focus-visible:ring-2 ${member.isGraduated
            ...
            aria-label={member.isGraduated ? `${member.name} 졸업 해제 및 복귀` : `${member.name} 졸업 처리`}
          >
            {member.isGraduated ? <RotateCcw size={18} aria-hidden="true" /> : <GraduationCap size={18} aria-hidden="true" />}
          </button>
        </div>

        <div className="mt-3 flex items-center gap-2 rounded-lg bg-slate-50 p-2 text-xs text-slate-500">
          <span className="flex-1 truncate font-mono select-all" title={member.channelId}>{member.channelId}</span>
          <button
            onClick={(event) => {
      ...
      <Card.Body className="space-y-4 pt-2">
        <section aria-labelledby={`ko-aliases-${String(member.id)}`}>
          <div id={`ko-aliases-${String(member.id)}`} className="mb-2 flex items-center gap-1 text-[11px] font-bold uppercase tracking-wider text-slate-400">
            <span className="h-1.5 w-1.5 rounded-full bg-sky-400" aria-hidden="true"></span>
            한국어 별명
          </div>
          <div className="mb-2 flex min-h-[24px] flex-wrap gap-1.5" role="list">
            ...
            {koAliases.length === 0 && (
              <span className="text-xs italic text-slate-300">등록된 별명이 없습니다</span>
            )}
          </div>
          <div className="flex gap-1.5">
            <Input
              value={koInput}
              onChange={(event) => { setKoInput(event.target.value) }}
              placeholder="별명 추가…"
              className="h-8 flex-1 border-slate-200 bg-slate-50 text-xs focus-visible:ring-2 focus-visible:ring-sky-200"
              onKeyDown={(event) => { if (event.key === 'Enter') handleAddKoAlias() }}
        ...
        <section aria-labelledby={`ja-aliases-${String(member.id)}`}>
          <div id={`ja-aliases-${String(member.id)}`} className="mb-2 flex items-center gap-1 text-[11px] font-bold uppercase tracking-wider text-slate-400">
            <span className="h-1.5 w-1.5 rounded-full bg-rose-400" aria-hidden="true"></span>
            일본어 별명
          </div>
          <div className="mb-2 flex min-h-[24px] flex-wrap gap-1.5" role="list">
          ...
          <div className="flex gap-1.5">
            <Input
              value={jaInput}
              onChange={(event) => { setJaInput(event.target.value) }}
              ...
              onKeyDown={(event) => { if (event.key === 'Enter') handleAddJaAlias() }}
