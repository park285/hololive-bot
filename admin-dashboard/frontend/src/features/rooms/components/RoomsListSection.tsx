  ...
  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div className="lg:col-span-2 space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-display font-bold text-slate-900">{listTitle}</h3>
          ...
          rooms.map((room) => (
            <div key={room} role="listitem" className="flex items-center justify-between px-6 py-4 hover:bg-slate-50 transition-colors group focus-within:bg-slate-50">
              <div className="flex items-center gap-3">
                <div className={clsx('w-2 h-2 rounded-full', indicatorClassName)} aria-hidden="true" />
      ...
      <Card className="sticky top-6">
        <div className="p-5 space-y-4">
          <h3 className="font-display font-bold text-slate-900 flex items-center gap-2">
            <Plus className={isBlacklist ? 'text-rose-500' : 'text-blue-500'} size={18} aria-hidden="true" /> {addTitle}
          </h3>
