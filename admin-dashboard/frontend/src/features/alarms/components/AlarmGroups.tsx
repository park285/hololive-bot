...
const chunkGroups = (groups: AlarmGroup[]): AlarmGroup[][] => {
	...
	for (let index = 0; index < groups.length; index += GROUPS_PER_ROW) {
		rows.push(groups.slice(index, index + GROUPS_PER_ROW));
	}
	return rows;
};
...
export const AlarmGroups = ({
	...
	const groupRows = useMemo(() => chunkGroups(visibleGroups), [visibleGroups]);
		...
		<div className="space-y-6">
			<VirtualList
				...
				getItemKey={(row, rowIndex) =>
					row.map(getGroupKey).join("|") || `alarm-row-${String(rowIndex)}`
				}
