...
const LIVE_GRID_COLUMNS = 4;

const chunkLiveStreams = (streams: Stream[]) => {
	const rows: Stream[][] = [];
	for (let index = 0; index < streams.length; index += LIVE_GRID_COLUMNS) {
		rows.push(streams.slice(index, index + LIVE_GRID_COLUMNS));
	}
	return rows;
};
				...
				renderItem={(row, rowIndex) => (
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
						{row.map((stream, columnIndex) => {
							...
							const streamIndex = rowIndex * LIVE_GRID_COLUMNS + columnIndex;
