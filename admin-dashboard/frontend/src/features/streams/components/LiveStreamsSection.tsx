...
const chunkStreams = (streams: Stream[]): Stream[][] => {
	const rows: Stream[][] = [];
	for (let index = 0; index < streams.length; index += STREAMS_PER_ROW) {
		rows.push(streams.slice(index, index + STREAMS_PER_ROW));
	}
	return rows;
};
					...
					getItemKey={(row, rowIndex) =>
						row
							.map((stream, columnIndex) =>
								getStreamKey(stream, rowIndex * STREAMS_PER_ROW + columnIndex),
							)
							.join("|") || `live-row-${String(rowIndex)}`
					}
